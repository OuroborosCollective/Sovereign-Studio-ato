...
echo "=== ERROR SCAN ==="
# Note: GHOST_PILOT_APP_PACKAGE is inherited from the environment
...
if [ -n "$GHOST_PILOT_APP_PACKAGE" ] && grep -Ei "ANR in $GHOST_PILOT_APP_PACKAGE" emulator_error.log; then
  echo "❌ App ANR detected"
  grep -Ei "ANR in $GHOST_PILOT_APP_PACKAGE" emulator_error.log | head -n 40
  exit 1
fi

echo "✅ Error scan passed"
