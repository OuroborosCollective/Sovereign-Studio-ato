...
### 2026-09-24 — Bitcoin graph × ScaNN × Wolfram runtime verification
Status: PARTIAL — Bitcoin application/ScaNN path verified; real full-chain Core ingest and complete release workflow remain open
Task: Finish the deterministic Bitcoin research pipeline from canonical UTXO/transaction graph through live ScaNN candidate retrieval, exact rescoring and bounded Wolfram verification.
Decisions: Keep Bitcoin Core/UTXO state as canonical truth; keep ScaNN as approximate candidate retrieval only; keep exact rescore independent; keep Wolfram as supplemental mathematical counter-check; require a persistent dedicated runner for the complete mainnet corpus.
Touched surfaces: Bitcoin Core RPC adapter, canonical UTXO/graph store, chain indexer, 40-D feature vector, live ScaNN adapter, shard-aware index manifest, Wolfram CAG contract, focused regressions, shipping mirrors, CI workflows, architecture documentation and Notion evidence page.
Evidence: Final PR head `c0ff4175b87a69dff586c8d28c8310c1c053882a`; Bitcoin contract job `107532964033` completed successfully with compile + **18/18** regression tests; the same job built and queried real ScaNN 1.4.2 and emitted `LIVE_SCANN_SMOKE=PASS` with receipt hash `ff1a14ed49e4cc08488512a64b9cd8d10124a0a09591832096a35fa559f60b6d`; Agent Runtime Tests `107532964392` completed successfully; Integration Plan and Boundary Ledger runs were successful on the synchronized exact-head workflow family. Wolfram kernel smoke tests returned `True`, `True`, `Sqrt[2]`.
Learned: Live ANN execution exposed and fixed the ScaNN docid contract; canonical persistence exposed and fixed exact prevout/fee ordering and reorg spend-pointer cleanup. A green Core step with empty secrets is not Core evidence: the latest CI log explicitly reported `BITCOIN_CORE_LIVE=NOT_CONFIGURED`.
Open: No real Bitcoin Core endpoint is currently configured in repository secrets, so no live mainnet chain tip or full-chain ingest is proven; the persistent `bitcoin-indexer` runner required for full-chain storage is not provisioned; Release Verification and Sovereign ChatGPT MCP were still running at the final readback.
Next safe step: Provision the authorized persistent Bitcoin indexer/Core boundary tracked by issue #2089, execute bounded Core-tip ingest first, then a resumable persistent full-chain ingest, serialized shard build/reload, real Wolfram CAG transport receipt and Notion Evidence Ledger readback before any merge.
