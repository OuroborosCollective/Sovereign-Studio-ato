...
Decisions: Repository execution is now Frontend → persisted Sovereign job → exactly one Agent Zero A2A task → Agent-Zero-owned repository access/checkout → shared workspace mutation. Sovereign GitHub credentials are rejected on execution and remain separate for readback/publication only. Generic job/toolchain and cognitive-swarm repository mutation paths fail closed; controller repository execution bypasses LLM/billing resolution and delegates directly to the canonical A2A route. Canonical/shipping backend mirrors remain byte-equal.
Evidence: Memory pre-read completed on `main@1ecde15912c732349732e2f823142bbcf6b47fb2`. Wolfram reachability model verified `GitHubOAuthCanReachExecuteRepository=False`, `AgentZeroCanReachExecuteRepository=True`, with the sole intended frontend path `FrontendMission → BackendJob → AgentZeroA2A → ExecuteRepository`. Exact-head PR #2011 backend run `35445898883` passed the full Agent Runtime Tests on `9661eed6e166d12fe726506aa529fb24ed08e9e8`; Release Verification had already passed revision integrity, required-gate contracts and TypeScript before this Memory commit.
Learned: Authentication/readback authority and execution authority must be structurally separate. A GitHub credential available to a session must never become an implicit repository executor or prerequisite for starting an Agent Zero mission.
Open: Final exact-head gates after this Memory commit, merge, immutable production release/readback, and deployed Sovereign UI → Agent Zero smoke creating one empty root `testfile` without Draft PR remain required.
