...
### 2026-09-24 — Single-Agent app path and persistent chat readback repair
Status: PARTIAL — source + local regressions verified; exact-head CI/runtime pending
Task: Remove the vNext command surface's accidental Swarm transport for normal chat, make Single-Agent starts non-blocking, route GitHub repository missions directly to the existing Agent-Zero single repository executor, and make the final assistant answer plus cancellation state recoverable from backend truth.
Decisions: `agentMode` is fixed to `single` for this product surface; normal chat uses a dedicated `/api/user/agent/single/run` endpoint returning `202` after persisted-run creation; GitHub repository missions use the existing `/api/user/agent/repository/run` Agent-Zero A2A path; single-run GET/Resume/Cancel use `/api/user/agent/single/runs/*`; cancellation persists as `BLOCKED` + `RUN_CANCELLED` and single-agent finalization checks that terminal cancellation before writing completion; only the bounded final `assistant_text` (max 8,000 chars) is persisted/read back, never raw model output or secrets. This is the Single-Agent adaptation of the intent described in open issue #1921; no Swarm executor or second truth store is introduced.
Touched surfaces: `src/features/control-surface-vnext/adapter/interface.ts`, `src/features/control-surface-vnext/adapter/production-adapter.ts`, `src/features/control-surface-vnext/adapter/repository-bound-adapter.ts`, `src/features/control-surface-vnext/hooks/useSwarmRun.ts`, `src/features/control-surface-vnext/hooks/useSovereignJob.ts`, `src/features/control-surface-vnext/App.tsx`, `src/features/control-surface-vnext/types/domain.ts`, `src/features/control-surface-vnext/controlSurfaceTruthContract.test.ts`, `backend/agent_runtime/cognitive_swarm_routes.py`, `backend/agent_runtime/cognitive_run_store.py`, `scripts/sovereign-backend/agent_runtime/cognitive_swarm_routes.py`, `scripts/sovereign-backend/agent_runtime/cognitive_run_store.py`, `backend/tests/test_cognitive_swarm_routes.py`.
Evidence: Fresh workspace `job-9f06b0a9874a` based on `main@6d661043fb1a9320bdb3c134ea2251497944ad7d`; `test_cognitive_run_store.py` 18/18, `test_repository_execution.py` 38/38, `test_agent_zero_a2a.py` 7/7, `git diff --check` SUCCESS; canonical/shipping mirror report for cognitive run store + cognitive routes reports 0 drift. Frontend/Node validation remains GitHub-Actions-owned. Local route collection is environment-limited where Flask is absent, so no false runtime claim is made.
Learned: The apparent endless build was partly caused by the app using a Swarm-named start route even for Single mode, while normal Single runs had no cancellable `jobId` and no durable assistant-message projection. Separating normal Single chat from repository Agent-Zero execution removes that transport ambiguity and gives the UI a stable persisted run identity before long work begins.
Open: Exact-head GitHub frontend/typecheck/build and backend route regression must finish green; production deployment/readback is still required before claiming the live app path fixed.
Next safe step: Create the Draft PR from the current exact workspace head, re-read its final exact SHA and required checks, then use the normal owner-gated merge/runtime verification.
...
### 2026-09-24 — vNext Draft-PR publication rehydration
Status: PARTIAL — persistence path verified locally; exact-head CI/runtime pending
Task: Close the #1922 vNext publication truth gap by rehydrating verified GitHub Draft-PR state from the existing backend evidence store instead of an adapter-local Map.
Decisions: Reuse the existing `github_draft_pr_readback` event and owner-scoped evidence-anchors route; validate the persisted source hash and exact head/draft/readback invariants; keep CI state separate; add no new table, GitHub client, Swarm path, auto-merge or deployment path.
Touched surfaces: `backend/agent_runtime/job_store.py`, `backend/agent_runtime/routes.py`, their shipping mirrors, `backend/tests/test_live_workspace_projection_store.py`, `src/features/product/runtime/sovereignAgentClient.ts`, `src/features/control-surface-vnext/adapter/production-adapter.ts`, and its truth contract test.
Evidence: Draft PR #2092 updated to exact head `99883c37be0064f9e0ffc5b5f0797d46c85e9cee`; `git diff --check` and backend compile passed; mirror diff count 0; existing repository/A2A/run-store regressions remain green; new publication readback regression is included, while Node/frontend validation is GitHub-Actions-owned. Continuity reported only the existing non-blocking ledger advisory.
