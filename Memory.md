...
### 2026-09-25 — Bitcoin Graph × ScaNN × Wolfram carried forward
Status: PARTIAL — current-main patch prepared; CI/runtime readback pending
Task: Carry the reviewed Bitcoin UTXO graph, ScaNN candidate/exact-rescore bridge, bounded Wolfram CAG checks, canonical SQLite ingest and index-manifest integration from stale PR #2088 onto current main.
Decisions: Keep the complete functional retrieval modules, their canonical/shipping mirrors, contract tests, and guarded workflow integration. Do not copy the stale `Memory.md`; preserve credential boundaries by using repository secrets only for the authorized Bitcoin Core endpoint and keep the full-chain job manually dispatched on the dedicated runner label.
Touched surfaces: `.github/workflows/bitcoin-full-chain-index.yml`, `.github/workflows/sovereign-agent-backend.yml`, `backend/agent_runtime/retrieval/bitcoin_*`, `scripts/sovereign-backend/agent_runtime/retrieval/bitcoin_*`, `backend/agent_runtime/retrieval/__init__.py`, `scripts/sovereign-backend/agent_runtime/retrieval/__init__.py`, `backend/tests/test_bitcoin_graph_scann_wolfram.py`, `backend/tests/test_scann_production_mirror.py`, and the architecture document.
Evidence: Source PR #2088 head `b728fd210889f602bb8182ccacd076ed3c5ef87d` had terminal green checks but was `mergeable_state=dirty` because its base was `6baf7dd7d2ab179f580be65cf2f29c40bcf34484` while current main had advanced. Fresh current-main workspace `job-df2dad18b588` materialized all functional source/test/workflow/doc paths; canonical and shipping Bitcoin modules are byte-identical in the source PR readback; the dedicated full-chain workflow requires nonempty Bitcoin Core URL/user/password GitHub secrets and never prints them. Existing PR test suite covers exact satoshi conservation, deterministic graph snapshots, fixed 40-D vectors, ANN candidate-only semantics with exact rescore, fail-closed unknown candidates, deterministic recall, bounded Wolfram checks, credential requirements, canonical SQLite spend/reorg behavior, resumable ingest, and contiguous content-addressed index manifests.
Learned: A divergent retrieval feature can be preserved cleanly by moving its functional surface—not stale operational history—onto current main and maintaining mirror parity plus explicit secret/runner boundaries.
Open: Current-main Draft PR publication is retrying after a transient remote credential boundary.
Next safe step: Publish the exact prepared history, require terminal CI, then merge only the exact green head.
