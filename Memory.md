...
Task: Repair the post-merge MCP validation failure without weakening the new Agent Zero shared-workspace mount gate.
Decisions: Keep the production mount preflight unchanged; update only the legacy unexpected-runtime-readback test so it stubs a verified no-op Agent Zero mount and still reaches the intentionally injected `OSError`.
Touched surfaces: `tools/sovereign-chatgpt-mcp/tests/test_coordinated_release_reconciler.py`.
...
Learned: Introducing an earlier fail-closed preflight changes the prerequisites of tests that intentionally inject later-stage failures; those fixtures must satisfy the earlier boundary rather than bypass or remove it.
Open: Exact-head PR gates, a successful main MCP image, coordinated target-system receipt, mount readback, and fresh protected Five-Path proof remain required.
Next safe step: Publish this focused Draft PR, require terminal exact-head CI, merge under Owner approval only when green, then let the main MCP/coordinated release rerun the real host-mount path.
