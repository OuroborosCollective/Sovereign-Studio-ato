...
Status: SOURCE_PATCHED; follow-up exact-head CI and deployed smoke pending.
Task: Enforce one repository executor: Frontend → persisted Sovereign repository job → exactly one Agent Zero A2A task → Agent-Zero-owned repository checkout → shared workspace → Sovereign evidence; GitHub OAuth remains readback/publication authority only.
Decisions: Repository start requests reject GitHub credentials and never clone in Sovereign; Cognitive Swarm and Controller Board now also fail closed on repository-backed resume before billing/model resolution, and the swarm executor itself rejects injected repository jobs/toolsets. Canonical/shipping execution mirrors stay byte-equal; Draft-PR publication remains a separate explicit GitHub action.
Evidence: Memory pre-read was completed. #2011 merged as `main@89e5303ec81b2e8091b0b5be413c04737b568f4e`; forensic readback of run `35445815554` attempt 1 proved the missed resume edge (`1 failed, 1304 passed, 1 skipped`) because `create_repository_swarm_tasks(...)` still existed in swarm resume. The follow-up workspace removes all active `create_repository_swarm_tasks(` calls from swarm/controller, removes the residual `github_access_token=None` execution argument, passes `git diff --check`, backend compile, Agent-Zero-only contract 4/4 and Controller contract 20/20, with mirror report `6c082a936b1f76d781dac7093f171e185888376235f7a5ef2e1823edca048751`.
Learned: Blocking only repository start routes is insufficient; persisted resume paths and internal executor parameters are authority edges too and must be unable to reacquire repository tools, billing, or GitHub execution authority.
Open: Publish the follow-up exact head, require full GitHub regression/security/release gates, merge only when green, then verify immutable production revision/digest and run the deployed UI → Agent Zero empty-root-`testfile` smoke with no OAuth execution, Swarm, credits, commit, or Draft PR.
Next safe step: Publish this exact repair head, read all CI to terminal, merge under Owner approval only if green, then perform production revision/digest and Agent Zero workspace readback.
