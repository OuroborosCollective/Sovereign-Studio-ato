...
### 2026-09-21 — Repository stall-clock truth repair
Change: Bind the repository A2A stall deadline to immutable job `created_at` instead of trigger-refreshed `updated_at`, with canonical/shipping parity and focused regression updates.
Learned: Real job `agent-33db1102b0c34588a980ea248d7a3a58` stayed `submitted` while repeated event/job updates advanced `updated_at`; the database BEFORE UPDATE trigger therefore made the intended 30-minute fail-closed deadline self-resetting. The Workspace REVISION UNVERIFIED/0-files view is separately explained by direct repository jobs having no `agent_runs` live-workspace evidence context before closeout.
Evidence: Live DB trigger `update_sovereign_agent_job_timestamp()` sets `NEW.updated_at = NOW()` on every row update; Agent Zero logs show the exact job actively reasoning in its shared workspace while the frontend reports only repeated `submitted` observations. Runtime main before patch is `d9ce105ba992a1c6d0e9ec59bebcc4535fcfd3c3` on backend digest `sha256:2343053c858d5bee57a36bb63a65d4556b37e2ba534bca475d2462ce43dfd77a`; exact-head CI remains required before merge/deploy.
