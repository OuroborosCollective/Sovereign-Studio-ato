...
### 2026-09-22 — Play Release repository permission binding
Änderung: PlayReleaseChat lädt vor einem Repository-Run den echten Branch-Commit-Head, bindet ihn als `expectedHeadSha` an den Agent-Zero-A2A-Submit und blockiert ohne gültigen SHA vor jedem Job-Start; ein Regression-Contract schützt diese Kausalgrenze.
Erkenntnis: Der beobachtete `repository_permission_unbound`-Blocker entstand nicht im Permission-Gate selbst, sondern durch einen Frontend-Vertragsbruch, der `expectedHeadSha` in dieser Lane unterschlug.
Evidence: PR #2061 head `44345b5cb83bcacf0137cae6e1946625edb19645`; `git diff --check` und Backend-Compile SUCCESS; Agent Runtime Tests SUCCESS; Revision Guardian, Boundary Ledger, Continuity und Integration Plan SUCCESS; Release Verification und MCP-Operator laufen noch auf demselben exact head.
