...
## 2026-09-22 - [Dynamic State Labels and Consistent Localization]
**Learning:** Interactive components (like a send/dispatch button) that can be temporarily locked or disabled based on state (e.g., executing or empty input) need dynamic `aria-label` and `title` attributes. Furthermore, mixing localizations (like German ARIA labels in an English component) confuses screen readers.
**Action:** Always provide state-dependent descriptive accessible names and native tooltips for interactive buttons. Additionally, verify the surrounding context language and use matching, consistent localization in all added accessible labels, tooltips, and status descriptions.
