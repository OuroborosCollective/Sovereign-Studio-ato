...
## 2025-10-24 - [Keyboard Focus Consistency on Custom Form Controls]
**Learning:** In complex interactive sidebars, custom buttons (e.g., chat suggestions, clear input triggers, and model pickers) often lose native focus indicators due to default resetting CSS (`focus:outline-none`). This drastically impairs accessibility for keyboard navigators who cannot see which element is active.
**Action:** Always ensure that custom interactive elements utilizing Tailwind CSS apply explicit `focus-visible` ring modifiers (e.g., `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/50`) to maintain clear, deliberate keyboard accessibility tracking.
