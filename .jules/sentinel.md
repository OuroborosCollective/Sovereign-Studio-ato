...
## 2026-07-28 - Regex Robustness for Multi-Layer Secret Masking
**Vulnerability:** Synchronizing secret masking patterns across environments (e.g., Cloudflare Workers vs. Web) revealed a collision between generic label-based rules (e.g., `auth: ...`) and specific protocol-based rules (e.g., `Authorization: Bearer ...`). The generic rule was too greedy, consuming parts of the specific rule's prefix and resulting in double-masking or broken log formats (e.g., `Authorization: **** ****`).
**Learning:** When using multiple regex rules for redaction, generic "keyword" rules must be guarded against interfering with more specific "structure" rules. Order of operations matters, but negative lookaheads are more robust for preventing collisions when patterns share common keywords.
**Prevention:** Use negative lookaheads (e.g., `(?!Bearer\b)`) in generic label-based masking regexes to prevent them from matching prefixes that are handled by more specific rules. Ensure patterns are synchronized across all application boundaries (proxy, backend, frontend) to maintain a consistent security perimeter.
