...
## 2026-08-01 - Harden Endpoints Against Unhandled AttributeError Crashes
**Vulnerability:** The `/api/toolchain/sandbox-plan` endpoint parsed JSON payloads directly into `b` using `request.get_json(force=True) or {}` without verifying the payload's type. A non-dictionary payload (such as a list) that evaluates to truthy (e.g., `['item']`) bypasses falsy fallback defaults. Subsequent attempts to invoke dictionary methods like `b.get("goal")` raise an unhandled `AttributeError`, causing a 500 Internal Server Error crash and potential stack trace leakage.
**Learning:** Flask's `request.get_json()` dynamically parses any valid JSON type, including JSON arrays. Checking for dictionary structures is mandatory when dictionary methods or operations are performed on the root object.
**Prevention:** Explicitly enforce dictionary type validation using `isinstance(b, dict)` immediately after parsing JSON request bodies. Reject malformed payloads with a 400 Bad Request to prevent unhandled AttributeError exceptions and framework-level 500 crashes.
