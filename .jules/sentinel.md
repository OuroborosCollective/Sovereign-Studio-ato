...
## 2026-07-30 - Secured In-Memory OAuth State Store against Memory Exhaustion DoS
**Vulnerability:** The in-memory OAuth state store allowed clients to initiate authorization requests generating random `state` keys that were stored indefinitely without any sweeping or bounds constraint. An attacker could flood the system with state requests, causing memory exhaustion and a Denial of Service (DoS) of the server.
**Learning:** Temporary session or state variables stored in-memory must be actively swept of expired entries and capped at a maximum limit. Furthermore, to strictly cap memory usage under high load, proactive eviction (such as FIFO pruning of oldest states) must be executed after inserting the state to ensure the collection size never exceeds the design threshold.
**Prevention:** Implement a dual-layer cleanup in the state-storing function: first proactively filter and delete expired keys, and second, if the storage size is still above the threshold, sort and evict the oldest active states to guarantee a strict, un-bypassable memory upper bound.
