...
## 2026-07-30 - Prevent Sensitive Credential Exposure in Repository Content Scans
**Vulnerability:** The repository content scanner (`scanRepoContent`) identified sensitive credentials (like API keys, tokens, or passwords) using regular expression matching, but included the exact raw matching code line as the finding's `evidence`. This would leak the actual secret to state management, UI components, and logs/reports.
**Learning:** Security analysis tools that scan files for secrets must never expose the raw scanned secrets in their findings or logs. Instead, findings evidence must be redacted at the source before being saved to local state or reported.
**Prevention:** Always pipe scanned evidence lines through a robust, central masking utility (such as `maskSecrets`) to strip sensitive matching parameters before adding them to scan findings reports or metadata objects.
