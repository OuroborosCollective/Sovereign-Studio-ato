...
## 2026-07-30 - Explicit Dictionary-Type Validation for Flask Toolchain JSON Payloads
**Vulnerability:** The `/api/toolchain/*` endpoints parsed incoming JSON payloads with `request.get_json(force=True)` but did not verify if the resulting object was a dictionary. If an attacker sent a malformed JSON type (such as a list, string, or integer), calling `b.get(...)` on the parsed object triggered an unhandled `AttributeError` 500 error, causing potential internal stack trace leakage and server crash.
**Learning:** Flask's `request.get_json()` can return any valid parsed JSON type, not just dictionaries. When working with dynamic request data, always explicitly assert the type of the root parsed JSON object using `isinstance(body, dict)` before calling any dictionary accessor methods.
**Prevention:** Implement strict type guards (e.g., `isinstance(b, dict)`) immediately after fetching any JSON request payload, rejecting malformed structures with a clean `400 Bad Request` or `422 Unprocessable Entity` response.
