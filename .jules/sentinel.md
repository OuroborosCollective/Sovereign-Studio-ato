...
## 2026-07-28 - Secure Toolchain endpoints against Path Traversal and Authorization Bypass
**Vulnerability:** The toolchain endpoints (`tc_github_read_file`, `tc_github_list_directory`, `tc_apply_patch_worker`, and `tc_create_draft_pr`) accepted user-supplied `path` values without validation. Because the path is appended to the GitHub API endpoint `/repos/{owner}/{repo}/contents/{path}`, an attacker could perform path traversal via `..` segments to reach other repositories or endpoints accessible by the server's privileged `_TC_GITHUB_TOKEN`, bypassing repo allowlists.
**Learning:** Checking parameter values against an allowlist (like owners/repos) is insufficient if path components allow traversal (`..`). Path-based parameters must always be validated and normalized early before being concatenated into URL paths.
**Prevention:** Implement a strict, central validation utility (`_tc_validate_path`) that normalizes separators and rejects any path containing traversal characters (`..`), absolute segments (`/`), or null bytes (`\0`) with a structured 403 Forbidden.
