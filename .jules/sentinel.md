...
## 2026-07-25 - Defense-in-Depth for Safe Logging Components
**Vulnerability:** The `SafeLogText` component relied entirely on a boolean `isSensitive` prop to decide whether to mask data. If a developer accidentally passed a string containing a secret (e.g., an error message or a token-containing URL) without setting `isSensitive=true`, the secret would be leaked to the UI in plain text.
**Learning:** Manual sensitivity flagging is prone to human error. A "safe" component should provide defense-in-depth by always running its content through a pattern-based `maskSecrets` utility as a baseline, regardless of explicit flags. Furthermore, internal error handlers (e.g., in catch blocks for telemetry transmission) must also mask their own error messages before logging to the console.
**Prevention:** Always apply pattern-based masking (`maskSecrets`) as a mandatory filter in logging/display components, even when explicit sensitivity flags are present. Mask error messages in catch blocks to prevent secrets stored in error objects (like URLs or payloads) from leaking into the console.
