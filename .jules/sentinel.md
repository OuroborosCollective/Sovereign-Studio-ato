...
## 2026-08-05 - In-Memory Rate Limiter Eviction Policy Alignment
**Vulnerability:** The in-memory rate-limiter check routine only deleted expired entries when the dictionary size exceeded 1,000, leaving it vulnerable to memory exhaustion DoS if a client generated distinct non-expired identifiers. Capping must strictly evict live entries if sweeping expired entries is insufficient.
**Learning:** When applying eviction limits *before* inserting the new entry in a transactional lock block, the sweep target must evict down to `cap - 1` (e.g., 999 entries) rather than the strict cap value, so that the post-insertion dictionary size remains perfectly bounded to the desired limit.
**Prevention:** Always account for entry insertion offsets when calculating eviction slice ranges in bounded collections to prevent off-by-one threshold leaks.
