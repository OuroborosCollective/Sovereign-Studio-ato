...
## 2026-07-30 - Hardened JSON Payload Verification in GitHub App routes
**Vulnerability:** GitHub App webhook and deduct credit routes parsed incoming request JSON payloads but did not validate that the parsed result is a dictionary before invoking `.get()`. Sending a malformed JSON payload (such as a list or basic primitive) caused unhandled AttributeError or TypeError exceptions, resulting in HTTP 500 server crashes and leakage of internal server tracebacks.
**Learning:** Even internal/admin endpoints or webhook integration vectors must never assume the root type of a client-supplied JSON payload. Enforcing explicit type checks protects endpoints from crashing due to unexpected input shapes.
**Prevention:** Always use an explicit dictionary-type guard `isinstance(payload, dict)` at the entrypoint of JSON-parsing route handlers before calling dictionary methods like `.get()`. Return a clear 400 Bad Request if the request is malformed.
