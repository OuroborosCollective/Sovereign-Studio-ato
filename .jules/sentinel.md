...
## 2026-07-27 - Secured In-Memory Rate Limiter against Memory Exhaustion DoS
**Vulnerability:** The in-memory rate limiter tracked request timestamps in a dictionary mapped to client identifiers (e.g. IP addresses). Although old timestamps were filtered out of lists, the dictionary keys for inactive identifiers were never deleted. An attacker could flood the server with requests using spoofed or rotating identifiers, permanently bloating the dictionary and causing a denial-of-service (DoS) via memory exhaustion.
**Learning:** State tracking structures used for anonymous client traffic must always be actively bounded. Simply empty-clearing list values is insufficient because the dictionary keys themselves will persist in Python's memory.
**Prevention:** Keys must be explicitly and immediately deleted from state tracking structures when their underlying tracking state becomes empty or inactive. Additionally, a proactive sweeping boundary must be implemented to prune stale keys when the dictionary grows beyond a safe predefined limit.

## 2026-07-28 - Explicit Dictionary-Type Verification for Flask get_json Payloads
**Vulnerability:** Publicly accessible MFA, security policy, and step-up authentication endpoints parsed incoming JSON payloads without checking the root payload type. An attacker could pass a malformed JSON type (such as list, string, or integer) instead of a dictionary, triggering unhandled AttributeError or TypeError 500 exceptions and potentially leaking technical server-side internals.
**Learning:** Flask's `request.get_json()` dynamically parses any valid JSON type, not just JSON objects (dictionaries). Always explicitly validate that the parsed payload is an instance of `dict` before invoking dictionary-specific methods like `.get()`.
**Prevention:** Ensure explicit type guards (e.g., `isinstance(body, dict)`) are implemented at the entry layer of all JSON-parsing routes to fail-closed with a 400 Bad Request error if a malformed payload type is submitted.

## 2026-07-29 - Robust Whitelist-Based URL Sanitizer with Advanced Obfuscation Removal
**Vulnerability:** Chat/markdown message link components relied on blocklist checks (e.g. `javascript:`, `data:`, `vbscript:`) to sanitize URLs. Attackers could bypass these checks via obfuscation vectors, such as embedding whitespace, control characters, null bytes, percent encoding (e.g. `%00`, `%09`), or JavaScript/HTML escapes, which are stripped by browsers during execution but bypass strict prefix matching.
**Learning:** A protocol blacklist is highly fragile and vulnerable to endless encoding/obfuscation variations. A strict protocol whitelist combined with aggressive normalization (stripping all whitespaces, control characters, percent-encoded, HTML entity-encoded, and backslash-escaped characters/control markers before validation) ensures consistent and bulletproof sanitization.
**Prevention:** Implement a strict protocol whitelist (allowing only `http:`, `https:`, `mailto:`, and `tel:`) paired with safe relative paths (e.g. starting with `/`, `#`, `./`, `../`). Perform full-spectrum obfuscation/escape removal during validation to preempt any browser-side normalization trick.

## 2026-08-01 - Default Dictionary Assignments and Truthy/Falsy Python JSON Payload Bypass
**Vulnerability:** Naive endpoint dict-default patterns (e.g., `body = request.get_json() or {}`) are fragile under non-dictionary types. In Python, empty lists/strings are falsy (evaluating to `{}`), but non-empty lists (e.g., `[1, 2]`), numbers, booleans, or strings are truthy. This bypassed default dict assignments, causing unhandled AttributeError/TypeError crashes on subsequent dictionary-only accessor methods like `.get()`.
**Learning:** Default assignments like `or {}` only protect against absent (None) or falsy payloads. Truthy non-dictionary values persist, bypass default value guards, and trigger unhandled server failures when processed downstream.
**Prevention:** Always parse the raw payload, explicitly assign an empty dictionary if the request is empty or None, and then perform a strict type guard (`isinstance(body, dict)`) to reject malformed payload configurations with an HTTP 400 Bad Request.

## 2026-08-04 - Strict Mirror Alignment and Scheme Integrity for Unified Security Contracts
...
## 2026-08-18 - Type Validation on Legacy LLM Provider Management Endpoints
**Vulnerability:** Admin and internal LLM provider management endpoints in `llm_provider_runtime.py` parsed JSON request bodies using `request.get_json(force=True) or {}` without verifying that the root payload was a dictionary. Sending a truthy non-dictionary JSON payload (e.g. `[1, 2]`) caused downstream `.get()` calls to fail with unhandled `AttributeError: 'list' object has no attribute 'get'` 500 crashes and potential stack trace exposure.
**Learning:** Default dictionary fallback expressions like `or {}` do not safeguard against truthy non-dictionary JSON inputs (such as non-empty arrays, numbers, or booleans). Raw JSON inputs must be checked against `isinstance(raw, dict)` before executing dictionary accessor operations.
**Prevention:** Always capture the parsed JSON payload into a temporary variable (`raw`), verify `isinstance(raw, dict)` (or allow `None` for optional bodies), and return a structured 400 Bad Request error if a non-dictionary payload type is received.
