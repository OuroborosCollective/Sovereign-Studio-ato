...
## 2026-08-27 - [Avoiding DOM Blob Allocations in String Byte Scanning]
**Learning:** Checking string byte size via `new Blob([content]).size` inside scanning loops instantiates heavy DOM Blob objects and buffer copies, imposing significant CPU and GC pressure. Utilizing string length fast-paths (`length > MAX` or `length <= MAX / 4`) combined with a module-level `TextEncoder` eliminates Blob allocations while preserving exact byte length measurement. Furthermore, tracking summary counts inline during loop execution avoids redundant $O(N)$ array filtering passes.
**Action:** Avoid `new Blob()` for string byte size checks in hot scanning routines; use string length guard bounds and `TextEncoder`. Accumulate summary statistics in the main loop pass instead of calling post-process `.filter()` or `.reduce()`.
