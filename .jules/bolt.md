...
## 2024-05-18 - [Optimizing Array String Inclusion Checks]
**Learning:** Checking a target string for the inclusion of ANY substring from an array using `Array.prototype.some` and `String.prototype.includes` is an O(N*M) operation that allocates closure and causes high loop overhead, especially noticeable on hot paths like routers.
**Action:** Replace `tokens.some(token => str.includes(token))` by precompiling a single, properly regex-escaped RegExp object mapped from the tokens (`new RegExp(tokens.map(escapeRegExp).join('|'))`) outside the function, and test using `.test(str)`. This pushes the search into the optimized native C++ regex engine and brings complexity to O(M). Remember to use `.cjs` for quick node script codemods since `"type": "module"` is used.
