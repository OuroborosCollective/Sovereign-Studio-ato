...
## 2026-08-01 - [Pre-computing lookup Sets for nested repository validations]
**Learning:** Sequential searches (`Array.prototype.some`) on a dynamic files list during UI component rendering (such as validation of suggestion gates) introduces $O(S \times F \times N)$ complexity, rendering components slowly on deep and large codebases. Transforming the files list into pre-computed Sets (`exactPaths` and `dirPrefixes`) inside the entry mapping stage reduces lookups to $O(1)$ complexity, speeding up suggestion-gating checks to $O(N + S \times F)$.
**Action:** Always pre-compute exact path and folder prefix Set lookup tables in a single $O(N)$ pass prior to mapping, filtering, or gating suggestions based on the repository structure.
