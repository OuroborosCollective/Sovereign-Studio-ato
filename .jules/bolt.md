...
## 2026-07-24 - [Memoizing List Orchestrations on Frequent Render Cycles]
**Learning:** Wrapping high-frequency or deep-render components like `ThoughtBubble` and `AgentEventStream` in `React.memo`, and memoizing complex collection operations (like sorting/filtering in `buildStream`) with `useMemo`, prevents expensive array sorting, deduplication, and subtree re-renders when parent states (like Controlled Text Inputs or interval monitors in `BuilderContainer`) fluctuate constantly.
**Action:** Always wrap list-processing components and status streams in `React.memo`, and use `useMemo` for any collection mapping/filtering/sorting performed inside their rendering scopes to prevent CPU and GC spikes.
