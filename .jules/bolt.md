...
## 2026-09-02 - [Consolidating Multi-Pass Array Traversals in UI Components]
**Learning:** Re-computing derived UI states (like category groupings and severity counts) using multiple chained \`.filter()\` calls on a large shared array (e.g., scan findings) inside a React component creates unnecessary intermediate arrays, adds GC pressure, and scales poorly ($O(N \times K)$). Consolidating these separate filters into a single-pass loop within a single \`useMemo\` reduces iteration complexity to $O(N)$ and completely avoids allocating throwaway arrays.
**Action:** When extracting multiple different sub-sets or computing various summary statistics from the same parent array in a UI component, replace independent \`.filter()\` and \`.reduce()\` chains with a single \`useMemo\` block containing a standard \`for...of\` loop to accumulate all results concurrently.
