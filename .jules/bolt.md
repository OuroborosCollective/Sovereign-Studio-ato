...
## 2026-08-28 - [Consolidating multi-pass array filters in counting loops]
**Learning:** Computing separate counts for different object properties using multiple chained `.filter(predicate).length` calls incurs unnecessary $O(N)$ array traversal and garbage collection pressure due to intermediate array allocations. Refactoring these multiple passes into a single `for` loop eliminates intermediate allocations and combines multiple O(N) traversals into a single pass.
**Action:** Always replace multiple `.filter(...).length` statements over the same array with a single-pass `for` loop that accumulates all necessary counts directly to avoid intermediate array allocation.
