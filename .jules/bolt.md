...
## 2024-08-15 - Replace `.some(includes)` with Hoisted RegExp
**Learning:** Found widespread anti-pattern: `TOKEN_ARRAY.some(t => str.includes(t))` inside high-frequency loops (e.g., repository path scanning & routing classifiers). While functionally correct, V8 executes compiled regex `(token1|token2)` significantly faster than looping over an array and invoking `String.includes` natively.
**Action:** Always pre-compile token arrays into hoisted Regular Expressions (ensuring to `escapeRegExp(token)` to prevent regex injection issues) when performing multi-substring match operations within hot loops or frequent routers.
