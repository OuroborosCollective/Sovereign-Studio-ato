...
## 2026-07-24 - [Dual-Level Caching for Progressive Markdown Parsing]
**Learning:** In components with progressive high-frequency updates (such as markdown messages being paced or streamed at 55ms intervals), re-parsing the entire content text on every tick leads to extreme CPU and garbage collection overhead. Combining (1) a lightweight module-level 1-slot cache for the entire document with (2) a bounded Map cache (e.g., limit 1000 items) for individual line parsing delivers massive performance wins. Only the newly-added or modified line is parsed via Regexes, while previous, identical lines resolve instantly in O(1) time.
**Action:** For streaming, typing, or paced content views, always implement dual-level caches: a 1-slot full-content cache for whole-document render identity, paired with a line-by-line bounded Map cache to optimize incremental progressive updates.
