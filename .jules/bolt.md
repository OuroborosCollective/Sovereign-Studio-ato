...
## 2026-08-13 - [Precompiling Array Patterns into Single Regex Expressions]
**Learning:** Checking a text against an array of strings (e.g. verbs, placeholders, documentation tokens) by dynamically creating `new RegExp(...)` inside `.some(...)` iterations introduces massive CPU overhead due to repeated RegExp instantiation. It can be 20x to 30x slower. Combining the array of search tokens into a single hoisted Regex like `new RegExp('(' + TOKENS.join('|') + ')', 'i')` drops this from an O(N) regex loop to an O(1) single `.test()` call.
**Action:** When matching strings against a static list of tokens or keywords in hot paths, avoid array iteration `.some(token => text.includes(token))` or dynamic RegExp construction. Instead, join the tokens into a single, hoisted, precompiled `RegExp` using the pipe `|` operator and run a single `.test()`.
