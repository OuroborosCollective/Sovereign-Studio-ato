...
## 2025-05-29 - [Consolidating Multi-Pass Scans for GitHub Tree Parsing]
**Learning:** Chained `.filter().map()` operations on potentially large GitHub file trees (e.g., in `devChatWorkerBridge.ts`) iterate over the entire tree multiple times and allocate large intermediate arrays. Crucially, a `.slice(0, 500)` placed in the middle of these chains fails to prevent the initial `.filter()` from processing every single item in the repository, resulting in unbounded $O(N)$ overhead where $N$ is the repository size.
**Action:** When bounding parsed tree results (e.g., to 500 files), use a single `for...of` loop with an explicit early `break`. This converts the $O(N)$ operation into a bounded $O(K)$ operation. Additionally, avoid `.split('/')` and use native `.indexOf('/')` and `.substring()` to prevent further array allocation overhead.
