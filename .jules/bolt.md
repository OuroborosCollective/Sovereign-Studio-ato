...
## 2026-09-02 - [Consolidating Multi-Pass Arrays and Avoiding Split in Path Extraction]
**Learning:** Extracting directory subsets from large file path arrays (like processing `blobPaths` via chained `.filter().map()` and `.map().filter()`) incurs unnecessary garbage collection overhead due to intermediate array allocations. When combined with `path.split('/')[0]` for top-level directory extraction, this can severely impact loop performance on deeply nested structures. By consolidating all parsing into a single `for...of` pass and using `indexOf('/')` combined with `substring()`, we completely bypass the creation of intermediate arrays and avoid the heap pressure of continuous string splitting.
**Action:** Always consolidate chained mapping and filtering logic on path arrays into single-pass loops. Replace `.split('/')` with native `indexOf('/')` and `substring()` operations when only the first or last component of a path is needed.
