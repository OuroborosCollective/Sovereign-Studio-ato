...
## 2026-08-05 - [Global localeCompare Cleanup in Hot Sorting Paths]
**Learning:** Even after partial optimization, remnants of slow, localization-heavy `localeCompare` string comparisons can persist in sorting routines across key modules. Replacing these callbacks with native lexicographical comparison operators (`<` and `>`) delivers a 10-20x speedup in Javascript engines (like V8) for identifier/path arrays. However, always inspect test suites for explicit locale-aware sorting requirements (such as handling of German umlauts) before globally replacing them.
**Action:** Replace `localeCompare` sorting callbacks with native `<` and `>` operators for identifier-based lists (paths, IDs, names), but preserve them where a strict internationalization/locale-aware sorting contract is verified by test cases.
