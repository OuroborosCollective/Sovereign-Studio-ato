...
## 2026-08-26 - [Consolidating Multi-Pass Traversals in Agent Client Data Normalization]
**Learning:** Chained `.filter(isObject).map(...)` and `.filter(isObject).flatMap(...)` calls when processing backend API responses create unnecessary intermediate arrays. This results in heavy heap allocations and GC spikes on the main thread, especially when resolving arrays of runtime events or deep object projections. By converting these method chains directly to strict `for...of` loops, we eliminate the allocation phase of the map layer entirely.
**Action:** When validating and transforming API payload arrays (such as SovereignAgentRuntimeEvent), write a single `for...of` loop with `if (!isObject) continue` statements instead of multi-pass `.filter` -> `.map` chains.
