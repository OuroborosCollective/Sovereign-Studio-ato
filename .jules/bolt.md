...
## 2026-08-27 - [Consolidating Multi-Pass Filtering and Regex Hoisting in Signal Extraction]
**Learning:** In signal and token extraction hot paths (such as `extractLearningTerms`), chaining multiple `.filter()` calls over arrays creates large intermediate arrays, causing significant heap allocations and garbage collection overhead. Furthermore, executing un-hoisted regular expression tests (like `/^\d+$/.test()`) inside these iterations recompiles the regex on every test. Consolidating the sequential filters into a single `for` loop and hoisting the regular expression to module scope completely eliminates intermediate array allocations and redundant regex compilation, keeping loop execution fast and flat.
**Action:** Always refactor chained `.filter()` or `.map()` methods into a single `for` loop to avoid intermediate allocations when dealing with high-frequency text tokenization. Always hoist regular expressions to module-level constants to avoid recompilation inside loops.
