...
## 2026-08-27 - [Regex Hoisting and Escaping for Array Token Matching]
**Learning:** Recompiling a regular expression on every invocation of a function, particularly inside an array '.some()' loop, incurs unnecessary runtime overhead. In routing files where static lists of string tokens are matched against input text, checking them iteratively using `.some(token => lower.includes(token))` is significantly slower than testing against a single module-scoped, compiled `RegExp`. Compiling array values into a single joined RegExp string (using an `escapeRegExp` helper and `.join('|')`) drastically improves performance and reduces garbage collection pressure by transforming multiple `O(N)` loop matches into a single `O(1)` regex execution.
**Action:** Always hoist array token lists into module-level `RegExp` constants (ensuring tokens are regex-escaped properly) and use the `.test()` method instead of `.some()` for optimal performance in high-frequency text validation and intent routing functions.
