...
## 2026-08-27 - [Single-Pass Loop Consolidation and Regex Hoisting in Gate Evaluations]
**Learning:** Evaluating change risk gates (such as merge blast radius) through chained `.filter(Boolean)`, `new Set()`, `[...Set]`, `.reduce()`, and `[...].some()` array operations allocates multiple intermediate arrays and closures on every evaluation tick. Hoisting regular expression constants to module scope and consolidating path deduplication, critical path classification, and dependency importer summation into a single `for...of` loop pass eliminates intermediate array allocations and reduces loop traversals to a single $O(N)$ pass.
**Action:** Always hoist regular expression definitions to module scope and consolidate multi-pass array filtering, set conversion, and reductions into a single `for...of` loop pass for gate evaluations.
