...
## 2026-09-20 - [Single-Pass Tree Parsing and Substring Decomposition]
**Learning:** Extracting metadata from large arrays of objects (like GitHub tree API responses) by chaining multiple \`.filter().map()\` operations and using \`.split('/')[0]\` for path decomposition introduces unnecessary O(N) multi-pass array allocations and extreme garbage collection pressure. Consolidating logic into a single-pass \`for...of\` loop, extracting top-level directories natively using \`.indexOf('/')\` and \`.substring()\`, and applying an explicit early bound limit via a \`Set\` minimizes execution time to a flat O(N) scan.
**Action:** Always replace chained \`.filter().map()\` operations with a single-pass loop when parsing large file trees, and use \`.indexOf()\` with \`.substring()\` over \`.split()\` when decomposing simple path elements to avoid intermediate array allocations.
