...
import { 
  ...
  Search, BookOpen, Flame, Beaker, Unlock, Settings
} from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="w-full h-[100dvh] flex flex-col bg-[#f3f3f2] text-stone-900 overflow-hidden text-sm">
      ...
      <button onClick={() => setShowConfig(true)} className="fixed right-6 bottom-6 p-3 bg-indigo-600 text-white rounded-full shadow-lg hover:bg-indigo-700 transition-all hover:scale-110 z-30 group"><Settings size={24} /></button>
      <ConfigBar isOpen={showConfig} onClose={() => setShowConfig(false)} config={{ repoUrl, ghPat, geminiKey }} onChange={() => {}} onSave={() => {}} onReset={() => {}} repoUrl={repoUrl} setRepoUrl={setRepoUrl} handleRepoChange={handleRepoChange} ghPat={ghPat} handleGhPatChange={handleGhPatChange} geminiKey={geminiKey} handleGeminiKeyChange={handleGeminiKeyChange} />
