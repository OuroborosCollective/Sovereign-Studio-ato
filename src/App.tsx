...
export default function App() {
  ...
  return (
    <div className="w-full h-[100dvh] flex flex-col bg-[#f3f3f2] text-stone-900 overflow-hidden text-sm">
      ...
      <PaywallModal isOpen={showPaywall} onSubscribe={async () => { setIsPro(true); await storageService.set('ss_is_pro', 'true'); setShowPaywall(false); }} onClose={() => setShowPaywall(false)} />
      <PrivacyModal isOpen={showPrivacy} onAccept={() => setShowPrivacy(false)} onClose={() => setShowPrivacy(false)} />
    </div>
  );
}