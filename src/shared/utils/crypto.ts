...
export function maskSecrets(text: string): string {
  ...
  masked = masked.replace(/(ghp|gho|ghu|ghs|ghr)_[a-zA-Z0-9]{8,100}/g, '$1_****');
  ...
  masked = masked.replace(/AIza[a-zA-Z0-9_-]{20,60}/g, 'AIza****');

  // AI provider style keys
  masked = masked.replace(/sk-[a-zA-Z0-9_-]{12,120}/g, 'sk-****');
  masked = masked.replace(/gsk_[a-zA-Z0-9_-]{12,120}/g, 'gsk_****');
