...
const LABEL_CREDENTIAL_REGEX = /(["']?)(client[_-]?secret|client[_-]?id|session[_-]?token|session[_-]?id|refresh[_-]?token|auth[_-]?token|id[_-]?token|api[_-]?secret|password|passwd|token|secret|api[_-]?key|access[_-]?token|private[_-]?key|access[_-]?key[_-]?id|secret[_-]?access[_-]?key|access[_-]?key|aws[_-]?secret[_-]?access[_-]?key|encryption[_-]?key|cipher[_-]?key|passphrase|database[_-]?password|db[_-]?password|db[_-]?pass|master[_-]?password|master[_-]?key|secret[_-]?key|signing[_-]?key|signing[_-]?secret)\1(\s*[:=]\s*)["']?[a-zA-Z0-9_@#$%^&*.\-~+/=]+["']?/gi;
