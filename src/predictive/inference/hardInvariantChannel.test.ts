...
describe('predictive/inference/hardInvariantChannel', () => {
  ...
  describe('checkHardInvariant', () => {
    ...
    it('fails closed when no bounds are defined', () => {
      const invariant: HardInvariant = {
        id: 'test_unknown',
        name: 'Unknown Metric',
        category: 'resource',
    ...
    it('fails closed for non-finite evidence', () => {
      const invariant: HardInvariant = {
        id: 'nan',
        name: 'Invalid Metric',
        category: 'resource',
      ...
      const result = checkHardInvariant(invariant, runtimeRevision);
  ...
  describe('runHardInvariantChannel', () => {
    ...
    it('returns an explicit failed receipt when required invariants are missing', () => {
      const config = createDefaultRuntimeInvariantConfig();
      config.invariants = [];

      const results = runHardInvariantChannel(config, mockRevisionBinding, 'missing_trace');

      expect(results).toHaveLength(1);
      expect(results[0].passed).toBe(false);
      expect(results[0].receipt.abortReason).toBe('No hard invariants configured');
      expect(results[0].traceId).toBe('missing_trace');
    });
