...
export function createModelReceipt(input: CreateReceiptInput): ModelReceipt {
  ...
  const completedReceipt: ModelReceipt = {
    ...receipt,
    receiptHash: computeReceiptHash(receipt),
  };
  const validation = validateReceipt(completedReceipt);
  if (!validation.isValid) {
    throw new TypeError(`Invalid model receipt: ${validation.errors.join('; ')}`);
  }
...
export function validateModelReceipt(
  ...
  if (ageMs < 0 || ageMs > maxAgeMs) {
    errors.push(`Receipt is stale: ${ageMs}ms old (max ${maxAgeMs}ms)`);
  }
...
export function createRevisionBinding(
  ...
  if (!/^[0-9a-f]{40}$/i.test(runtimeRevision)) {
    throw new TypeError('runtimeRevision must be a full Git SHA');
  }
  if (!configRevision.trim()) {
    throw new TypeError('configRevision is required');
  }
  return {
    runtimeRevision,
    configRevision,
...
export function createInputWindowHash(
  ...
  if (
    ...
    throw new TypeError('Input window timestamps are invalid');
  }
  if (signals.some(signal => !Number.isFinite(signal.timestamp) || !Number.isFinite(signal.value))) {
    throw new TypeError('Input window signals must be finite');
  }

  return {
    hash: hashCanonical({ signals, windowStart, windowEnd }),
    signalCount: signals.length,
    windowStart,
    windowEnd,
    featureHash: hashCanonical(signals.map(signal => signal.value)),
  };
}
...
export function createModelStateHash(
  ...
  if (!libraryVersion.trim()) {
    throw new TypeError('libraryVersion is required');
  }
  return {
    parametersHash: hashCanonical(parameters),
    weightsHash: hashCanonical(weights),
