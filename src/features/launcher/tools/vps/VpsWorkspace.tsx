  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div 
        ...
        role="tablist"
        aria-label="VPS Workspace Ansichten"
      >
        {([
          { id: 'tree' as MobileTab, icon: Folder, label: 'Dateien', panelId: 'vps-file-tree-panel' },
          { id: 'chat' as MobileTab, icon: MessageSquare, label: 'Chat', panelId: 'vps-chat-panel' },
        ] as const).map(({ id, icon: Icon, label, panelId }) => (
          <button
            key={id}
            id={`vps-tab-${id}`}
            type="button"
            ...
            title={label === 'Dateien' ? 'Dateibaum anzeigen' : 'Chat anzeigen'}
            style={{
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
        ...
        <div style={{
          ...
          className="md:flex"
          role="tabpanel"
          id="vps-file-tree-panel"
          aria-labelledby="vps-tab-tree"
        >
          <div style={{
