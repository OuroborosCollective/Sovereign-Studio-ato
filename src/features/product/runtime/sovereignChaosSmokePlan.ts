...
function assertStepValid(step: SovereignChaosSmokeStep): void {
  ...
  if (!step.liveSafe) throw new Error(`Chaos smoke step ${step.id} must stay test-only and live safe.`);
}
  ...
  const plan: SovereignChaosSmokePlan = {
    ...
    steps: [
      ...
      },
      {
        ...
        action: 'Record dependency failures, wait deterministic cooldown, then record a successful half-open probe.',
