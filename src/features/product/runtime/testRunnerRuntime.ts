...
const SIGNATURES: Record<TestFramework, readonly string[]> = { pytest: ['::test_', 'pytest', ' passed', ' failed', 'PASSED', 'FAILED'], jest: ['PASS ', 'FAIL ', 'Tests:', 'jest'], vitest: ['vitest', 'Test Files', 'Tests  ', '✓', '×'], go_test: ['--- PASS', '--- FAIL', 'ok  \t', 'FAIL\t', 'go test'], cargo_test: ['test result:', 'cargo test', 'running '], unknown: [] };
export function detectFrameworkFromOutput(output: string): TestFramework { for (const [framework, signatures] of Object.entries(SIGNATURES)) if (framework !== 'unknown' && signatures.some((value) => output.includes(value))) return framework as TestFramework; return 'unknown'; }
