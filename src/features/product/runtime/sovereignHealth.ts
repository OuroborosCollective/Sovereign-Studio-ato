...
export function buildSovereignHealthReport(input: SovereignHealthInput): SovereignHealthReport {
  ...
  const telemetryWarnings = input.telemetry?.events.filter((event) => event.level === 'warning').length ?? 0;

  const criticalRisks = highIntegrity + highGenerated + workflowRed + telemetryErrors;
  const totalIssues = criticalRisks + mediumIntegrity + mediumGenerated + workflowPending + telemetryWarnings;
