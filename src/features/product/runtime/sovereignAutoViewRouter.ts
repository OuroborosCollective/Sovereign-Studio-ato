...
const SIDE_TABS = new Set<SovereignAutoViewTab>(['memory', 'remote', 'telemetry', 'readiness', 'integrity', 'findings', 'health', 'runtime', 'coverage']);
const DEFAULT_AUTO_SWITCH_INACTIVITY_MS = 3_000;
const DEFAULT_PATTERN_CONFIDENCE_THRESHOLD = 0.8;
...
export function isSovereignAutoViewUserInactive(input: SovereignAutoViewInput, thresholdMs = input.autoSwitchInactivityMs ?? DEFAULT_AUTO_SWITCH_INACTIVITY_MS): boolean {
  const nowMs = normalizeNowMs(input);
  if (nowMs === null || !isFiniteNumber(input.lastUserInteractionAt)) return true;
  return nowMs - input.lastUserInteractionAt >= thresholdMs;
}

export function isSovereignAutoViewPatternMatched(input: SovereignAutoViewInput, threshold = input.patternConfidenceThreshold ?? DEFAULT_PATTERN_CONFIDENCE_THRESHOLD): boolean {
  ...
  return conditions.every((condition) => {
    switch (condition.type) {
      ...
      case 'PATTERN_MATCHED':
        return isSovereignAutoViewPatternMatched(input, condition.confidenceThreshold);
...
function canRunSuggestionSwitch(input: SovereignAutoViewInput): boolean {
  ...
  return isSovereignAutoViewPatternMatched(input);
}
...
function switchTo(input: SovereignAutoViewInput, tab: SovereignAutoViewTab, reason: string): SovereignAutoViewDecision {
  return {
    shouldSwitch: tab !== input.activeTab,
    tab,
    reason,
  };
}
...
export function validateSovereignAutoViewInput(input: SovereignAutoViewInput): string[] {
  ...
  if (input.workflowStatus && !VALID_WORKFLOW_STATUSES.includes(input.workflowStatus)) {
...
export function decideSovereignAutoView(input: SovereignAutoViewInput): SovereignAutoViewDecision {
  ...
  if (input.activeStep) {
    ...
    return switchTo(input, tab, `Active runtime step ${input.activeStep} owns the ${tab} view.`);
  }

  if (input.isPublishing) {
    return switchTo(input, 'workflow', 'Draft PR publishing should show the workflow/log view.');
  }

  if (input.isWatchingWorkflow) {
    return switchTo(input, 'workflow', 'Workflow watch is running and should stay visible.');
  }

  if (input.workflowStatus === 'red') {
    return switchTo(input, 'repair', 'Red workflow status should surface the repair view.');
  }

  if (input.workflowStatus === 'pending' || input.workflowStatus === 'unknown') {
    return switchTo(input, 'workflow', 'Non-final workflow status should stay on workflow watch.');
  }

  if (SIDE_TABS.has(input.activeTab) && input.mode === 'manual') {
    return keepCurrent(input, 'Manual mode keeps intentional side tabs visible when nothing is running.');
  }

  const canSuggest = canRunSuggestionSwitch(input);
  if (!canSuggest) {
  ...
  if (input.workflowStatus === 'green' && input.hasPackage) {
    if (input.hasDiffSources) {
      return switchTo(input, 'diff', 'Green workflow with diff sources loaded - show the diff.');
    }

    return switchTo(input, 'files', 'Green workflow with generated package - show files view.');
  }

  if (input.hasPackage && input.hasDiffSources && input.workflowStatus === 'idle') {
    return switchTo(input, 'diff', 'Package ready with diff sources - review the generated diff.');
  }

  if (input.hasPackage && input.mode !== 'manual') {
    return switchTo(input, 'files', 'Auto mode generated package is ready for review.');
  }

  if (input.hasPackage && input.workflowStatus === 'idle' && input.mode === 'manual') {
    if (MAIN_FLOW_TABS.has(input.activeTab)) {
      return switchTo(input, 'workflow', 'Package ready in manual mode - you can start workflow from here.');
    }

    return keepCurrent(input, 'Manual mode keeps side tabs visible after package generation.');
  }

  if (input.hasActivePatterns && input.hasPackage) {
    return switchTo(input, 'memory', 'Active patterns available with package - show learned patterns.');
  }

  if (input.hasActiveTelemetry && !SIDE_TABS.has(input.activeTab)) {
    return switchTo(input, 'telemetry', 'Active telemetry events - show status.');
  }
