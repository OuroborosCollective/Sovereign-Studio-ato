...
function projectionArray(value: unknown): SovereignLiveProjection[] {
  ...
  return value.filter(isObject).flatMap((item): SovereignLiveProjection[] => {
    ...
    if (!['IDE_FILE', 'IDE_DIFF', 'TERMINAL', 'WINDOW_FOCUS'].includes(kind)) return [];
    if (!['REQUESTED', 'VISIBLE', 'UNAVAILABLE', 'STALE'].includes(state)) return [];
    if (!['REPOSITORY', 'GIT', 'PROCESS'].includes(source)) return [];
    const projectionId = stringValue(item.projectionId);
    ...
    if (!projectionId || !jobId || !workspaceId || !actionId || !sourceReceiptRef || !sourceIdentityHash || !projectionHash) return [];
    return [{
      projectionId,
      jobId,
      workspaceId,
