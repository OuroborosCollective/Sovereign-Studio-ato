...
export function buildEvidenceLineage(entries: readonly EvidenceLineageInput[]): EvidenceLineageChain[] {
  ...
  return [...groups.entries()]
    // Fast native lexicographical string comparison replacing slow localeCompare to avoid V8 collation overhead
    .sort(([left], [right]) => (left < right ? -1 : left > right ? 1 : 0))
    .map(([scope, scopedEntries]) => {
      // Fast native lexicographical string comparison for stable ID ordering
