...
function parseGitHubUrl(url: string): { owner: string; repo: string; branch: string | null } | null {
  const match = url.trim().match(/github\.com\/([^/?#]+)\/([^/?#]+?)(?:\.git)?\/?(?:tree\/([^?#]+))?(?:[?#].*)?$/i);
  if (!match) return null;
  return {
    owner: match[1],
    repo: match[2].replace(/\.git$/, ''),
    branch: match[3] ? decodeURIComponent(match[3].replace(/\/+$/, '')) : null,
  };
}

function encodeGitBranchPath(branch: string): string {
...
export function useProductMagic() {
  ...
  const mergeWhenGreen = useCallback(async () => {
    ...
    if (!tokenValidation.valid) {
      setIsWorking(false);
      log('FEHLER: ' + (tokenValidation.errors[0] || tokenValidation.error || 'GitHub Token fehlt'));
    ...
    try {
      ...
      const repoData = await verifyResponse.json();
      const defaultBranch = typeof repoData.default_branch === 'string' && repoData.default_branch.trim()
        ? repoData.default_branch.trim()
        : 'main';
      let baseBranch = parsed.branch || defaultBranch;

      // Create a new branch for the changes
      const timestamp = Date.now();
      const branchName = `feature/sovereign-studio-${timestamp}`;
      
      // Get the source branch SHA. If a stale /tree/<branch> URL is stored, fall back to the real default branch.
      let refResponse = await fetch(
        `https://api.github.com/repos/${parsed.owner}/${parsed.repo}/git/ref/heads/${encodeGitBranchPath(baseBranch)}`,
        { headers }
      );

      if (!refResponse.ok && refResponse.status === 404 && parsed.branch && parsed.branch !== defaultBranch) {
        log(`Branch '${parsed.branch}' nicht gefunden. Nutze Default-Branch '${defaultBranch}'.`);
        baseBranch = defaultBranch;
        refResponse = await fetch(
          `https://api.github.com/repos/${parsed.owner}/${parsed.repo}/git/ref/heads/${encodeGitBranchPath(baseBranch)}`,
          { headers }
        );
      }
      ...
      if (!refResponse.ok) {
        if (refResponse.status === 404) {
          throw new Error(`Branch '${baseBranch}' nicht gefunden. Bitte Repo-URL oder Default-Branch pruefen.`);
        }
      ...
      const baseCommitResponse = await fetch(
        `https://api.github.com/repos/${parsed.owner}/${parsed.repo}/git/commits/${baseSha}`,
        { headers }
      );

      if (!baseCommitResponse.ok) {
        throw new Error(`Konnte Basis-Commit nicht laden: ${baseCommitResponse.status}`);
      }

      const baseCommitData = await baseCommitResponse.json();
      const baseTreeSha = baseCommitData.tree?.sha;
      if (!baseTreeSha) {
        throw new Error('Basis-Commit enthaelt keinen Tree-SHA.');
      }
      ...
      if (!newTreeResponse.ok) {
        throw new Error(`Konnte Tree nicht erstellen: ${newTreeResponse.status}`);
      }

      const newTreeData = await newTreeResponse.json();
      if (!newTreeData.sha) {
        throw new Error('GitHub lieferte keinen neuen Tree-SHA.');
      }
      ...
      if (!commitResponse.ok) {
        throw new Error(`Konnte Commit nicht erstellen: ${commitResponse.status}`);
      }

      const commitData = await commitResponse.json();
      if (!commitData.sha) {
        throw new Error('GitHub lieferte keinen neuen Commit-SHA.');
      }

      // Update branch reference
      const updateRefResponse = await fetch(
        `https://api.github.com/repos/${parsed.owner}/${parsed.repo}/git/refs/heads/${encodeGitBranchPath(branchName)}`,
        {
          method: 'PATCH',
          headers,
          body: JSON.stringify({
      ...
      if (!updateRefResponse.ok) {
        throw new Error(`Konnte Branch-Referenz nicht aktualisieren: ${updateRefResponse.status}`);
      }
