...
export const MainContent: React.FC<MainContentProps> = ({
  ...
  isWorking = false,
  agentMessage = 'Bereit fuer den naechsten manuellen Schritt.',
  progress = 0
}) => {
  ...
  return (
    <section className="flex-1 min-w-0 flex flex-col bg-stone-50">
      ...
      <div className="flex-1 min-h-0 p-3 sm:p-4 overflow-hidden flex flex-col gap-3">
        <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-3 text-xs text-indigo-950 shadow-sm">
          <div className="flex gap-3">
            <div className={`h-10 w-10 rounded-2xl flex items-center justify-center shrink-0 ${isWorking ? 'bg-indigo-600 text-white animate-pulse' : 'bg-white text-indigo-700 border border-indigo-200'}`}>
              {isWorking ? <Loader2 size={20} className="animate-spin" /> : <Bot size={20} />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 font-black"><span className={`w-2.5 h-2.5 rounded-full ${red ? 'bg-red-500 animate-pulse' : 'bg-stone-300'}`} /><span className={`w-2.5 h-2.5 rounded-full ${isWorking ? 'bg-yellow-400 animate-pulse' : 'bg-stone-300'}`} /><span className={`w-2.5 h-2.5 rounded-full ${green ? 'bg-emerald-500 animate-pulse' : 'bg-stone-300'}`} /> Agent spricht</div>
              <p className="mt-1 mb-0 leading-relaxed">{agentMessage}</p>
              <div className="mt-2 h-2 rounded-full bg-white border border-indigo-100 overflow-hidden"><div className={`h-full bg-indigo-600 transition-all duration-700 ${isWorking ? 'animate-pulse' : ''}`} style={{ width: `${Math.max(4, Math.min(100, progress))}%` }} /></div>
              <div className="mt-1 text-[10px] font-bold text-indigo-700">{isWorking ? 'Ich arbeite aktiv an deinem Auftrag. Bitte warten.' : 'Ein Schritt nach dem anderen. Kein Auto-Endloslauf.'}</div>
            </div>
          </div>
        </div>
      ...
      <div className="h-12 bg-white border-t border-stone-200 flex items-center px-3 sm:px-4 gap-3 shrink-0">
        ...
        <button disabled={isWorking} onClick={sendChat} className="bg-indigo-600 disabled:bg-stone-300 text-white px-4 py-1.5 rounded text-[10px] font-bold uppercase shadow-sm"><Send size={13}/></button>
      </div>

      <div className="border-t border-indigo-200 px-3 sm:px-4 py-2 flex items-center justify-between bg-indigo-50 shrink-0 gap-3">
        <div className="flex-1 min-w-0"><h4 className="text-[10px] font-black text-indigo-800 uppercase">Auftrag: {cardsCount} Schritte · Fixlauf {fixLoops}/{settings.maxFixLoops}</h4><input readOnly value={agentMessage} className="w-full text-[10px] p-1 border border-indigo-200 rounded bg-white" /></div>
        <div className="flex gap-2 shrink-0"><button disabled={isWorking} onClick={publishAndValidate} className="px-4 py-2 bg-indigo-600 disabled:bg-stone-300 text-white rounded-lg font-bold text-[10px] uppercase">Pruefen</button><button disabled={isWorking} onClick={patchFromPipeline} className="px-4 py-2 bg-amber-500 disabled:bg-stone-300 text-white rounded-lg font-bold text-[10px] uppercase">Fix</button><button disabled={isWorking} onClick={mergeWhenGreen} className="px-4 py-2 bg-emerald-600 disabled:bg-stone-300 text-white rounded-lg font-bold text-[10px] uppercase">Frei</button></div>
      </div>
    </section>
  );
};
