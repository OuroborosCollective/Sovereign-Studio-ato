    ...
    <section className="flex-1 min-w-0 flex flex-col bg-stone-50">
      ...
      <div className="flex-1 min-h-0 p-3 sm:p-4 overflow-hidden flex flex-col gap-3">
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-2 h-2 rounded-full bg-white border border-indigo-100 overflow-hidden">
                <div className={`h-full transition-all duration-700 ${derivedWorking ? 'animate-pulse bg-indigo-600' : 'bg-stone-400'}`} style={{ width: `${Math.max(4, Math.min(100, derivedProgress))}%` }} />
              </div>
              <div className="mt-1 text-[10px] font-bold text-indigo-700">
                {derivedWorking ? 'Ich arbeite aktiv an deinem Auftrag. Bitte warten.' : 'Ein Schritt nach dem anderen. Kein Auto-Endloslauf.'}
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 min-h-[260px] rounded-xl shadow-inner overflow-hidden flex flex-col bg-stone-950 font-mono border border-stone-800">
          <div className="h-9 bg-stone-900 border-b border-stone-800 flex items-center gap-2 px-3 text-[10px] text-stone-400">
            ...
            <span className="ml-2">Matrix File Editor · {derivedWorking ? 'arbeitet...' : 'bereit'}</span>
          </div>
      ...
      <div className="border-t border-indigo-200 px-3 sm:px-4 py-2 flex items-center justify-between bg-indigo-50 shrink-0 gap-3">
        ...
        <div className="flex gap-2 shrink-0">
          <button disabled={derivedWorking} onClick={publishAndValidate} className="px-4 py-2 bg-indigo-600 disabled:bg-stone-300 text-white rounded-lg font-bold text-[10px] uppercase">Pruefen</button>
          <button disabled={derivedWorking} onClick={patchFromPipeline} className="px-4 py-2 bg-amber-500 disabled:bg-stone-300 text-white rounded-lg font-bold text-[10px] uppercase">Fix</button>
          <button disabled={derivedWorking} onClick={mergeWhenGreen} className="px-4 py-2 bg-emerald-600 disabled:bg-stone-300 text-white rounded-lg font-bold text-[10px] uppercase">Frei</button>
        </div>
      </div>
    </section>
  );
