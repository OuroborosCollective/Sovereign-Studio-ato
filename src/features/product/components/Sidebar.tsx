...
import { Rocket, Wand2, Plus } from 'lucide-react';
  ...
  return (
    <section className="w-full md:w-64 shrink-0 border-r border-stone-200 bg-white flex flex-col">
      <div className="p-4 bg-indigo-50 border-b border-indigo-100 shrink-0">
        ...
        <button onClick={buildProduct} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded text-[11px] font-bold uppercase shadow-sm flex items-center justify-center gap-2"><Rocket size={14}/> Auftrag starten</button>
      </div>
      ...
      <div className="p-3 bg-stone-50 border-b border-stone-200 shrink-0">
        ...
        <div className="flex gap-2 mt-2">
          <button onClick={buildProduct} className="flex-1 bg-stone-800 hover:bg-black text-white py-1.5 rounded text-[11px] font-bold uppercase shadow-sm"><Wand2 size={13} className="inline mr-1"/>Uebernehmen</button>
          <button onClick={addCard} className="shrink-0 bg-yellow-100 hover:bg-yellow-200 border border-yellow-300 text-yellow-800 py-1.5 px-3 rounded text-[11px] font-bold uppercase shadow-sm"><Plus size={13} className="inline mr-1"/>Notiz</button>
        </div>
      </div>
