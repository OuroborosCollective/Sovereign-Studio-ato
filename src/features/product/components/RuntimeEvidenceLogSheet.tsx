...
import { LOG_LEVEL_ICON, LOG_LEVEL_LABEL, STATUS_FONT_SIZE_MIN } from '../styles/statusTokens';
  ...
  return (
    <div role="presentation" onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 84, background: 'rgba(14,17,22,0.84)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      <section role="dialog" aria-modal="true" aria-label="Runtime Evidence Logs" onClick={(event) => event.stopPropagation()} style={{ width: '100%', maxWidth: 680, maxHeight: '78vh', overflowY: 'auto', margin: '0 auto', borderRadius: '20px 20px 0 0', border: `1px solid ${C.border}`, background: C.surface, padding: '16px 16px calc(22px + env(safe-area-inset-bottom, 0px))' }}>
        ...
        {lineages.length > 0 && <div data-testid="evidence-lineage-lens" style={{ marginTop: 10, display: 'grid', gap: 6 }}>{lineages.map((lineage) => <div key={lineage.scope} style={{ border: `1px solid ${C.sky}33`, borderRadius: 9, padding: 8, background: `${C.sky}08` }}><div style={{ color: C.sky, fontSize: 9, fontFamily: 'monospace' }}>{lineage.scope}</div><div style={{ color: C.textSub, fontSize: 10, marginTop: 3 }}>{lineage.nodes.map((node) => node.source).join(' → ')}</div></div>)}</div>}
        {entries.length === 0 ? <p style={{ color: C.textMuted }}>Noch keine Runtime-Ereignisse.</p> : <ol style={{ listStyle: 'none', padding: 0, margin: '12px 0 0', display: 'grid', gap: 8 }}>{entries.map((entry) => <li key={entry.id} data-runtime-source={entry.source} style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: 9, background: C.bg }}><div style={{ color: C.textMuted, fontSize: 9, fontFamily: 'monospace' }}>{new Date(entry.at).toLocaleTimeString('de-DE')} · {entry.source} · {entry.scope}</div><div style={{ color: entry.level === 'error' ? C.rose : entry.level === 'warning' ? C.amber : entry.level === 'success' ? C.green : C.text, fontSize: STATUS_FONT_SIZE_MIN, marginTop: 4, display: 'flex', alignItems: 'baseline', gap: 6 }}><span role="img" aria-label={LOG_LEVEL_LABEL[entry.level]} title={LOG_LEVEL_LABEL[entry.level]} style={{ fontSize: STATUS_FONT_SIZE_MIN, lineHeight: 1 }}>{LOG_LEVEL_ICON[entry.level]}</span><span>{entry.message}</span></div></li>)}</ol>}
      </section>
    </div>
  );
}
