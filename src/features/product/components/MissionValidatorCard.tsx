...
export function MissionValidatorCard({ result, onContinue, onEdit }: MissionValidatorCardProps) {
  const scoreText = `Validierungs-Score: ${result.score} von 100`;
  return (
    <section className="mx-3 my-2 rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-sm" data-testid="mission-validator-card">
      ...
      {result.questions.length > 0 && (
        <ul
          ...
          role="list"
          aria-label="Vorschläge zur Ergänzung"
        >
          {result.questions.map((question) => (
            <li key={question} role="listitem">
              {question}
            </li>
          ))}
        </ul>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          ...
          onClick={onEdit}
          aria-label="Mission mit weiteren Details ergänzen"
        ...
        <button
          ...
          onClick={onContinue}
          aria-label="Mission trotz Warnungen starten"
          title="Mission trotz Warnungen starten"
        >
          Trotzdem starten
        </button>
      </div>
