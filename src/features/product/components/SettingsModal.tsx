    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-stone-200">
        <div className="p-4 bg-indigo-600 text-white flex items-center justify-between">
          <h3 className="text-sm font-black uppercase tracking-widest flex items-center gap-2"><Shield size={16}/> System Konfiguration</h3>
          <button onClick={() => setShowSettings(false)} className="text-indigo-200 hover:text-white font-bold text-lg">×</button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-[10px] font-black text-stone-500 uppercase mb-1">GitHub Repository URL</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-black text-stone-500 uppercase mb-1">GitHub Personal Token</label>
              <input type="password" value={accessKey} onChange={(e) => setAccessKey(e.target.value)} className="w-full p-2 text-[12px] border border-stone-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="ghp_..." />
            </div>
            <div>
              <label className="block text-[10px] font-black text-stone-500 uppercase mb-1">Gemini API Key</label>
              <input type="password" value={geminiKey} onChange={(e) => setGeminiKey(e.target.value)} className="w-full p-2 text-[12px] border border-stone-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="AI-Key" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 pt-2">
            ...
            <div>
              <label className="block text-[10px] font-black text-stone-500 uppercase mb-1">Project Mode</label>
          ...
          <div className="bg-indigo-50 p-3 rounded-xl border border-indigo-100 mt-2">
            <h4 className="text-[10px] font-black text-indigo-800 uppercase flex items-center gap-1 mb-1"><Sparkles size={12}/> Agent Spezialisierung</h4>
        ...
        <div className="p-4 bg-stone-50 border-t border-stone-200">
          <button onClick={() => setShowSettings(false)} className="w-full bg-stone-900 text-white py-2 rounded-xl text-[11px] font-black uppercase shadow-lg hover:bg-black transition-all">Konfiguration Speichern</button>
        </div>
      </div>
    </div>
  );
