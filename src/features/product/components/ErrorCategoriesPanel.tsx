...
function categoryIcon(category: ScanFindingCategory): React.ReactElement {
  if (category === 'security-leak' || category === 'runtime-guard') return <Shield size={16} className="text-red-300" aria-hidden="true" />;
  if (category === 'auth') return <Lock size={16} className="text-orange-300" aria-hidden="true" />;
  if (category === 'type-error') return <Type size={16} className="text-purple-300" aria-hidden="true" />;
  if (category === 'ci-failure' || category === 'workflow') return <Zap size={16} className="text-yellow-300" aria-hidden="true" />;
  if (category === 'build-logic') return <Wrench size={16} className="text-blue-300" aria-hidden="true" />;
  if (category === 'test-doubles') return <Bug size={16} className="text-green-300" aria-hidden="true" />;
  if (category === 'architecture') return <Eye size={16} className="text-cyan-300" aria-hidden="true" />;
  if (category === 'docs') return <Code size={16} className="text-stone-300" aria-hidden="true" />;
  return <FileWarning size={16} className="text-slate-300" aria-hidden="true" />;
}
    ...
    <section aria-label="Fehlerkategorien Übersicht" className={`rounded-2xl border border-slate-700/60 bg-slate-900 ${className}`}>
      <header className="border-b border-slate-700/70 px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          ...
          >
            {activeFindings.length} aktiv · {resolvedCount} gelöst
          </span>
        </div>
      </header>
