...
describe('SovereignSessionRoadmap Accessibility Enhancements', () => {
  ...
  it('renders the compact view with comprehensive accessible tooltip and label attributes', () => {
    ...
    const compactContainer = screen.getByRole('group', {
      name: 'Session-Roadmap: 1 von 3 Schritten abgeschlossen. Aktueller Schritt: Schritt 2, 1 blockiert',
    });
    expect(compactContainer).toHaveAttribute('data-testid', 'roadmap-compact');
    ...
    // Health indicator uses a nameable live status role; the visual icon stays decorative.
    const healthIndicator = screen.getByRole('status', { name: 'Status: Warnung' });
    expect(healthIndicator).toHaveAttribute('title', 'Status: Warnung');
    expect(screen.getByText('⚠')).toHaveAttribute('aria-hidden', 'true');
  });

  it('announces a completed session without inventing a current step', () => {
    const session = createMockSession();
    const completedSession = {
      ...
      plan: {
        ...session.plan!,
        steps: session.plan!.steps.map((step) => ({ ...step, status: 'completed' as const })),
      },
    };
    const progress = {
      completed: 3,
    ...
    const compactContainer = screen.getByRole('group', {
      name: 'Session-Roadmap: 3 von 3 Schritten abgeschlossen. Alle Schritte abgeschlossen; kein aktiver Schritt',
    });
    expect(compactContainer).toHaveAttribute(
      'title',
      'Session-Roadmap: 3 von 3 abgeschlossen. Alle Schritte abgeschlossen; kein aktiver Schritt'
    );
    expect(compactContainer).not.toHaveAccessibleName(/N\/A/);
    expect(screen.getByText('Kein aktiver Schritt')).toBeInTheDocument();
  });
});
