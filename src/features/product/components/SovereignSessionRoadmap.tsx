...
export const SovereignSessionRoadmap: React.FC<SovereignSessionRoadmapProps> = ({
  ...
  if (compact) {
    ...
    const currentTitle = currentStep?.title ?? 'Kein aktiver Schritt';
    const currentStepSummary = currentStep
      ? `Aktueller Schritt: ${currentTitle}`
      : completed === total
        ? 'Alle Schritte abgeschlossen; kein aktiver Schritt'
    ...
    return (
      <div
        ...
        aria-label={`Session-Roadmap: ${completed} von ${total} Schritten abgeschlossen. ${currentStepSummary}${blocked > 0 ? `, ${blocked} blockiert` : ''}`}
        title={`Session-Roadmap: ${completed} von ${total} abgeschlossen. ${currentStepSummary}${blocked > 0 ? `, ${blocked} blockiert` : ''}`}
        style={{
          padding: '4px 12px',
          fontSize: '11px',
          color: '#666',
      ...
      >
        <span role="status" aria-label={`Status: ${healthText}`} title={`Status: ${healthText}`}>
