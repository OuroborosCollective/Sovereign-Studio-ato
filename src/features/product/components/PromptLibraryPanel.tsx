...
export function PromptLibraryPanel({ onSelectTemplate, onClose }: { readonly onSelectTemplate: (prompt: string) => void; readonly onClose: () => void }) {
  const storage = typeof window === 'undefined' ? null : window.localStorage; const [state,setState] = useState(() => storage ? loadPromptLibraryState(storage) : { version:1 as const,customTemplates:[],savedAt:0 }); const [query,setQuery] = useState(''); const [category,setCategory] = useState<PromptCategory | undefined>(); const [add,setAdd] = useState(false); const [label,setLabel] = useState(''); const [prompt,setPrompt] = useState(''); const [newCategory,setNewCategory] = useState<PromptCategory>('custom');
  const templates = useMemo(() => filterTemplates(getAllTemplates(state),{ query,category }),[state,query,category]); const categories = useMemo(() => availableCategories(getAllTemplates(state)),[state]);
  return (
    <div
      role="dialog"
      aria-modal="true"
