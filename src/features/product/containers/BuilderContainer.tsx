  ...
  return (
    <div className="absolute inset-0 z-[90] flex" data-testid="sovereign-devchat-side-menu">
      ...
      <div className="flex w-[min(10vw,9rem)] min-w-[8.5rem] flex-col border-l border-slate-800 bg-[#0d1117] py-4 max-lg:w-[220px]">
        ...
        <div className="mt-auto space-y-2 border-t border-slate-800 px-3 pt-3"><button type="button" className="w-full rounded-xl border border-slate-700 px-3 py-2 text-left text-xs text-slate-200" onClick={() => { onGenerateIdeas(); onClose(); }} data-role={SOVEREIGN_ACTION_ANALYZE_MISSION.dataRole} data-testid={SOVEREIGN_ACTION_ANALYZE_MISSION.testId} aria-label={SOVEREIGN_ACTION_ANALYZE_MISSION.ariaLabel}>Interne Prüfung</button><button type="button" className="w-full rounded-xl border border-amber-500/40 px-3 py-2 text-left text-xs text-amber-100" onClick={() => { onGenerateErrorWorkflow(); onClose(); }}>Fehleranalyse</button><button type="button" className="w-full rounded-xl border border-cyan-500/40 px-3 py-2 text-left text-xs text-cyan-100" onClick={() => { onPublishDraftPr(); onClose(); }}>{builderPublishLabel(isPublishing)}</button></div>
      </div>
    </div>
  );
}
  ...
  return (
    <section className={`${builderContainerContract.rootClass} sovereign-builder-compact relative mt-4 overflow-hidden rounded-3xl border border-cyan-400/25 bg-black text-sm text-slate-200 shadow-2xl shadow-cyan-950/10`} data-role={builderContainerContract.dataRole} data-testid={builderContainerContract.testId} data-layout="devchat-runtime-shell" aria-label={builderContainerContract.ariaLabel}>
      ...
      <div className="grid min-h-[72vh] grid-cols-1 gap-4 px-4 py-4 lg:grid-cols-[minmax(8rem,1fr)_minmax(0,55vw)_minmax(8rem,1fr)]" data-layout-zone="devchat-with-side-menus">
        ...
        <div className="min-w-0"><div className={centerColumnClass} data-chat-body-width="55%"><div className="mb-2 px-1"><div className="flex flex-wrap items-center gap-2 text-sm"><span className="h-2.5 w-2.5 rounded-full bg-emerald-300" aria-hidden="true" /><h2 className="text-lg font-black text-slate-50">Sovereign Chat</h2><span className="rounded-full border border-slate-800 bg-slate-950 px-2 py-1 text-xs text-slate-400">OpenHands Runtime</span></div><div className="mt-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-3 text-xs text-slate-400" aria-label="Sovereign geführter Chat Ablauf">
  <p><strong className="text-slate-200">Schritt 1</strong> · Repo per Chat oder Repo-Menü verbinden.</p>
  <p><strong className="text-slate-200">Schritt 2</strong> · Auftrag analysieren und echten Repo-Kontext prüfen.</p>
  <p><strong className="text-slate-200">Schritt 3</strong> · Agent starten oder Stopper klar anzeigen.</p>
</div><div className="mt-3 flex flex-wrap items-center gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Sovereign Arbeitsbereiche">{WORKBENCH_PANES.map((pane) => <button key={pane.id} type="button" aria-pressed={activePane === pane.id} className={activePane === pane.id ? 'flex-shrink-0 rounded-full border border-cyan-300/50 bg-slate-900 px-3 py-2 text-sm font-bold text-slate-50' : 'flex-shrink-0 rounded-full border border-transparent bg-transparent px-3 py-2 text-sm font-bold text-slate-500'} onClick={() => setActivePane(pane.id)}><span className="mr-1 text-slate-400">{pane.icon}</span>{pane.label}</button>)}</div><div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-600"><p>{paneHelpText(activePane)}</p><button type="button" className="rounded-full border border-cyan-400/40 px-3 py-1 font-bold text-cyan-100 disabled:border-slate-800 disabled:text-slate-500" onClick={analyzeWish} disabled={!wishText.trim()} data-role={SOVEREIGN_ACTION_ANALYZE_MISSION.dataRole} data-testid={SOVEREIGN_ACTION_ANALYZE_MISSION.testId} aria-label={SOVEREIGN_ACTION_ANALYZE_MISSION.ariaLabel}>Schritt 2 · Auftrag analysieren</button></div></div>
