...
type ModuleId     = 'chat' | 'init' | 'router' | 'pattern' | 'sync' | 'orchestr' | 'logger';
type SignalType   = 'idle' | 'active' | 'processing' | 'warning' | 'error';
type AnimPhase    = 'idle' | 'spinup' | 'working' | 'completing' | 'done' | 'error';
...
const STATUS_COLOR: Record<AgentStatus, string> = {
  idle: C.green, thinking: C.sky, editing: C.amber, running: C.violet, error: C.rose,
};
const STATUS_LABEL: Record<AgentStatus, string> = {
  idle: 'bereit', thinking: 'denkt…', editing: 'editiert', running: 'läuft', error: 'fehler',
};
const TIER_COLOR: Record<RuntimeTier, string> = {
  ready: C.green, active: C.sky, blocked: C.rose,
};
...
const INIT_CONDITIONS: Partial<Record<ModuleId, ModuleCond[]>> = {
  init:    [{ label: 'Module loaded',     status: 'pass' }, { label: 'Config valid',      status: 'pass' }],
  router:  [{ label: 'Signal ACTIVE',     status: 'pass' }, { label: 'No override',       status: 'pass' }, { label: 'Tab completed',    status: 'wait' }],
  pattern: [{ label: 'Seq ≥ 2',          status: 'pass' }, { label: 'Confidence ≥ 0.80', status: 'fail' }, { label: 'Store > 0',        status: 'wait' }],
  sync:    [{ label: 'Signal ACTIVE',     status: 'pass' }, { label: 'Inactivity > 3s',   status: 'wait' }, { label: 'Override clear',   status: 'pass' }],
  orchestr:[{ label: 'All tabs ready',    status: 'wait' }, { label: 'AutoSwitch ON',     status: 'pass' }, { label: 'Pattern matched',  status: 'fail' }],
  logger:  [{ label: 'Logger active',     status: 'pass' }, { label: 'Buffer not full',   status: 'pass' }],
};
...
function buildOutcomeHints(job: OpenHandsJobSnapshot | undefined): ChatOutcomeHint[] {
  ...
  const files = (job.changedFiles ?? []).filter((f) => typeof f === 'string' && f.trim()).map((f) => f.trim());
  const draftPrUrl = safeHttpsUrl(job.draftPrUrl);
  if (job.openHandsId?.trim()) hints.push({ kind: 'runtime', text: `🐤 OpenHands ID: ${job.openHandsId.trim()}` });
...
function deriveAgentStatus(args: {
  readonly repoBusy: boolean; readonly runtimeBusy: boolean; readonly isPublishing: boolean;
  readonly openhandsIsRunning?: boolean; readonly openhandsJob?: OpenHandsJobSnapshot;
  readonly localRepoLoading: boolean; readonly localRepoError: boolean;
}): AgentStatus {
...
interface PALDecision { tier: 'fast'|'smart'|'power'; modelId: string; modelLabel: string; score: number; costFactor: number }
const ARCH_KW  = ['architektur','architecture','refactor','redesign','migration','pattern','dependency','abstraction','interface','contract'];
const PLAN_KW  = ['plan','planung','roadmap','strategie','feature','implement','konzept','vorschlag','analyse','überblick'];
const QUICK_KW = ['kurz','quick','schnell','simple','einfach','was ist','what is','define','erkläre','explain','tipp'];
const THINK_KW = ['denk nach','think','tiefgründig','trade-off','kompromiss','komplexität','algorithmus','optimiere'];

function palRoute(message: string, histDepth: number, fileCount: number, prior: PALDecision[]): PALDecision {
  const lower = message.toLowerCase();
  let score = 0;
  const len = message.length;
  if (len >= 300) score += 15; else if (len >= 150) score += 10; else if (len >= 60) score += 5;
  ...
  const tier: 'fast'|'smart'|'power' = score <= 33 ? 'fast' : score <= 66 ? 'smart' : powerCount >= 10 ? 'smart' : 'power';
  const tierModelMap: Record<string, string[]> = { fast: ['llama-3-8b','gemma-7b'], smart: ['qwen-14b','llama-3.1-8b'], power: ['deepseek-r1','mistral-7b'] };
  const matched = DEV_CHAT_WORKER_MODELS.find(m => tierModelMap[tier].includes(m.id)) ?? DEV_CHAT_WORKER_MODELS[0];
  const costMap  = { fast: 1, smart: 10, power: 30 };
  return { tier, modelId: matched?.id ?? 'llama-3-8b', modelLabel: matched?.label ?? 'Llama 3 8B', score, costFactor: costMap[tier] };
}

function sameRecord<T extends string>(a: Record<string, T>, b: Record<string, T>): boolean {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
      {(['idle','thinking','editing'] as AgentStatus[]).map((s) => (
        <span key={s} style={{ display: 'inline-block', width: 7, height: 7, borderRadius: '50%', background: status === s ? STATUS_COLOR[s] : `${STATUS_COLOR[s]}30`, boxShadow: status === s ? `0 0 6px ${STATUS_COLOR[s]}` : 'none', transition: 'all 0.3s' }} />
      ))}
...
function TopBar({
  ...
  const repoLabel = chatRepoSnapshot ? `${chatRepoSnapshot.name}:${chatRepoSnapshot.branch}` : repoReady ? 'Repo ✓' : 'Repo fehlt';
      ...
      <div style={{ height: 52, display: 'flex', alignItems: 'center', padding: '0 12px', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 700, color: C.text, letterSpacing: -0.3 }}>Sovereign</span>
            <span style={{ fontFamily: 'monospace', fontSize: 9, padding: '2px 6px', borderRadius: 10, background: `${C.accent}18`, color: C.accent, border: `1px solid ${C.accent}33` }}>DevChat</span>
            {/* PAL badge */}
            {palTier && (
...
function StatusPanel({
  ...
  const levelColor: Record<string, string> = { info: C.sky, signal: C.green, warn: C.amber, error: C.rose, debug: C.textMuted };
    ...
    <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', borderBottom: `1px solid ${C.border}` }}>
        {(['logs','signals'] as const).map(t => (
          <button key={t} type="button" onClick={() => setTab(t)} style={{ flex: 1, height: 28, background: 'transparent', border: 'none', borderBottom: `2px solid ${tab === t ? C.green : 'transparent'}`, color: tab === t ? C.text : C.textMuted, fontFamily: 'monospace', fontSize: 9.5, cursor: 'pointer', transition: 'color 0.15s' }}>
            {t.toUpperCase()}
          </button>
        ))}
        {/* Clear logs button */}
      ...
      <div style={{ height: 88, overflowY: 'auto', padding: '4px 10px', position: 'relative' }}>
        {tab === 'logs' && [...logs].reverse().slice(0,25).map((e, i) => (
          <div key={i} style={{ display: 'flex', gap: 6, fontFamily: 'monospace', fontSize: 9.5, lineHeight: 1.65 }}>
            ...
            <span style={{ color: levelColor[e.level] ?? C.textMuted, width: 44, flexShrink: 0, textAlign: 'right' }}>{e.level.toUpperCase()}</span>
            <span style={{ color: C.textSub }}>{e.msg}</span>
          </div>
        ))}
        {tab === 'signals' && modules.filter(m => m.id !== 'chat').map(m => {
          const sig = signals[m.id] ?? 'idle';
          return (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', borderBottom: `1px solid ${C.border}` }}>
              <span style={{ display: 'inline-block', width: 7, height: 7, borderRadius: '50%', background: sig !== 'idle' ? m.color : `${m.color}28` }} />
              <span style={{ fontFamily: 'monospace', fontSize: 9.5, color: m.color, width: 56 }}>{m.id.toUpperCase()}</span>
              <span style={{ fontFamily: 'monospace', fontSize: 9.5, color: C.textSub, flex: 1 }}>{sig}</span>
            </div>
          );
        })}
      </div>
...
function OutcomeHints({ hints }: { hints: ChatOutcomeHint[] }) {
  ...
  return (
    <div style={{ padding: '0 12px 8px' }}>
      <div style={{ borderRadius: 10, border: `1px solid ${C.border}`, background: C.surface, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        {hints.map((h) => (
          <div key={`${h.kind}:${h.text}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, fontSize: 12, color: C.textSub }}>
            <span style={{ color: C.border, marginTop: 2, flexShrink: 0 }}>›</span>
            {h.href ? <a href={h.href} target="_blank" rel="noreferrer" style={{ color: C.sky, textDecoration: 'underline', textUnderlineOffset: 3 }}>{h.text}</a> : h.text}
          </div>
        ))}
      </div>
    </div>
...
function WelcomeScreen({ onIdea }: { onIdea: (opt: IdeaOption) => void }) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 20px', textAlign: 'center' }}>
      ...
      <h2 style={{ fontFamily: 'monospace', fontSize: 20, fontWeight: 800, color: C.text, marginBottom: 8, letterSpacing: -0.5 }}>Let&apos;s build!</h2>
      <p style={{ fontSize: 13, color: C.textSub, lineHeight: 1.6, maxWidth: 300, marginBottom: 28 }}>
        Schreib dein Ziel oder füge eine GitHub-URL ein. Sovereign prüft Gates und handelt nur bei echten Stop-Punkten.
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, width: '100%', maxWidth: 340 }}>
        {IDEA_OPTIONS.map((opt) => (
          <button key={opt.label} type="button" onClick={() => onIdea(opt)} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: '14px 12px', fontFamily: 'monospace', fontSize: 11, color: C.text, fontWeight: 600, cursor: 'pointer', textAlign: 'left', transition: 'border-color 0.15s, background 0.15s', lineHeight: 1.3 }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = C.borderHov; (e.currentTarget as HTMLButtonElement).style.background = '#1c2630'; }}
...
function ModuleScreen({
  ...
  conditions: Partial<Record<ModuleId,ModuleCond[]>>; confidence: number;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 14 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: `${mod.color}18`, border: `1px solid ${mod.color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17, color: mod.color }}>{mod.icon}</div>
        <div>
          <div style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 700, color: C.text }}>{mod.id.toUpperCase()}</div>
          <div style={{ fontFamily: 'monospace', fontSize: 9, color: C.textMuted }}>{mod.id} · {sig}</div>
        </div>
        <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
          <div style={{ fontFamily: 'monospace', fontSize: 10, color: phaseColor[phase] }}>{phase}</div>
          <div style={{ fontFamily: 'monospace', fontSize: 9, color: C.textMuted }}>{phaseSub[phase]}</div>
        </div>
      </div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '10px 12px' }}>
        <div style={{ fontFamily: 'monospace', fontSize: 8, color: C.textMuted, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Condition Chain</div>
        {conds.map((c, i) => {
          const cfg = { pass:{icon:'✓',color:C.green,bg:'rgba(52,211,153,0.08)'}, fail:{icon:'✗',color:C.rose,bg:'rgba(251,113,133,0.08)'}, wait:{icon:'⏳',color:C.amber,bg:'rgba(251,191,36,0.08)'} }[c.status];
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 8, background: cfg.bg, border: `1px solid ${cfg.color}22`, marginBottom: 4 }}>
              <span style={{ color: cfg.color, fontSize: 11, width: 14, textAlign: 'center' }}>{cfg.icon}</span>
              <span style={{ fontFamily: 'monospace', fontSize: 10.5, color: C.textSub, flex: 1 }}>{c.label}</span>
              <span style={{ fontFamily: 'monospace', fontSize: 9, padding: '2px 6px', borderRadius: 4, color: cfg.color, border: `1px solid ${cfg.color}44` }}>{c.status.toUpperCase()}</span>
            </div>
          );
        })}
      </div>
      ...
      {/* Sequence */}
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: '10px 12px' }}>
        <div style={{ fontFamily: 'monospace', fontSize: 8, color: C.textMuted, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>Sequence</div>
          ...
          {sequence.slice(-7).map((s, i, arr) => {
            ...
            return (
              <React.Fragment key={i}>
                <div style={{ position: 'relative', flexShrink: 0, padding: '3px 7px', borderRadius: 4, fontFamily: 'monospace', fontSize: 9, color: col, border: `1px solid ${col}44`, background: `${col}10` }}>
                  {s.tabId.slice(0,3).toUpperCase()}
                  {s.auto && <span style={{ position: 'absolute', top: -5, right: -4, fontSize: 7, background: col, color: '#000', borderRadius: 2, padding: '0 2px' }}>A</span>}
                </div>
                {i < arr.length - 1 && <span style={{ color: C.border, fontSize: 10 }}>›</span>}
              </React.Fragment>
            );
          })}
          {sequence.length === 0 && <span style={{ fontFamily: 'monospace', fontSize: 9, color: C.textMuted }}>no events</span>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div onClick={onClose} style={{ position: 'absolute', inset: 0, zIndex: 100, background: 'rgba(14,17,22,0.85)', backdropFilter: 'blur(8px)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: C.surface, borderRadius: '20px 20px 0 0', border: `1px solid ${C.border}`, borderBottom: 'none', padding: '0 0 24px' }}>
        ...
        <div style={{ fontFamily: 'monospace', fontSize: 9, textAlign: 'center', color: C.textMuted, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 12 }}>Runtime Quelle</div>
        {sources.map((s) => (
          <button key={s.id} type="button" onClick={onClose} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', border: 'none', borderLeft: `3px solid ${s.id === current.id ? TIER_COLOR[s.tier] : 'transparent'}`, cursor: 'pointer', background: s.id === current.id ? `${TIER_COLOR[s.tier]}08` : 'transparent' } as React.CSSProperties}>
            <span style={{ width: 9, height: 9, borderRadius: '50%', background: TIER_COLOR[s.tier], boxShadow: `0 0 6px ${TIER_COLOR[s.tier]}`, flexShrink: 0 }} />
            <span style={{ flex: 1, textAlign: 'left' }}>
              <span style={{ display: 'block', fontFamily: 'monospace', fontSize: 12, color: C.text }}>{s.label}</span>
              <span style={{ fontFamily: 'monospace', fontSize: 9, color: C.textMuted }}>{s.description}</span>
            </span>
            {s.id === current.id && <span style={{ color: TIER_COLOR[s.tier], fontSize: 12 }}>✓</span>}
          </button>
        ))}
      </div>
    </div>
...
function SideDrawer({
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 90, display: 'flex' }}>
      <div onClick={onClose} style={{ flex: 1, background: 'rgba(14,17,22,0.7)', backdropFilter: 'blur(4px)' }} />
      <div style={{ width: 'min(80vw, 300px)', background: C.surface, borderLeft: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,0.5)' }}>
        {/* Header */}
        <div style={{ padding: '16px 16px 12px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: `${C.accent}12`, border: `1px solid ${C.accent}33`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>⬡</div>
          <div>
        ...
        {/* Repo info */}
        {chatRepoSnapshot && (
          <div style={{ margin: '12px 12px 0', padding: '10px 12px', borderRadius: 10, background: `${C.green}08`, border: `1px solid ${C.green}22` }}>
            <div style={{ fontFamily: 'monospace', fontSize: 10, fontWeight: 600, color: C.green }}>{chatRepoSnapshot.name}</div>
            <div style={{ fontFamily: 'monospace', fontSize: 9, color: C.textSub, marginTop: 2 }}>{chatRepoSnapshot.branch} · {chatRepoSnapshot.fileCount} files</div>
          </div>
        )}
        ...
        <div style={{ margin: '8px 12px 0', padding: '10px 12px', borderRadius: 10, background: C.bg, border: `1px solid ${C.border}` }}>
          <div style={{ fontFamily: 'monospace', fontSize: 9, color: C.textMuted, marginBottom: 4 }}>Cloudflare Workers</div>
          <div style={{ fontFamily: 'monospace', fontSize: 8, color: C.textMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{SOVEREIGN_WORKER_CHAT}</div>
          <div style={{ fontFamily: 'monospace', fontSize: 8, color: C.textMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{SOVEREIGN_WORKER_KV}</div>
        </div>
        ...
        <div style={{ flex: 1, padding: '12px 12px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {openhandsIsRunning && onCancelOpenHands && (
            <button type="button" onClick={() => { onCancelOpenHands(); onClose(); }} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(251,49,85,0.07)', border: '1px solid rgba(251,49,85,0.25)', color: C.rose, fontFamily: 'monospace', fontSize: 12, fontWeight: 600, cursor: 'pointer', textAlign: 'left' }}>✕ Agent stoppen</button>
          )}
        </div>
        <div style={{ padding: '12px' }}>
          <button type="button" onClick={() => { onPublishDraftPr(); onClose(); }} data-role={SOVEREIGN_ACTION_DRAFT_PR.dataRole} data-testid={SOVEREIGN_ACTION_DRAFT_PR.testId} aria-label={SOVEREIGN_ACTION_DRAFT_PR.ariaLabel} style={{ width: '100%', padding: '14px', borderRadius: 14, background: C.orange, border: 'none', color: '#fff', fontFamily: 'monospace', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: `0 4px 16px ${C.orange}40` }}>
...
function BottomTabBar({
  ...
  return (
    <nav style={{ height: 56, background: C.bg, borderTop: `1px solid ${C.border}`, display: 'grid', gridTemplateColumns: `repeat(${modules.length}, 1fr)`, flexShrink: 0 }} aria-label="Sovereign Studio Tabs">
      {modules.map((tab) => {
        const active = tab.id === activeTab;
        const sig    = (signals[tab.id] ?? 'idle') as SignalType;
        return (
          <button key={tab.id} type="button" onClick={() => onTabClick(tab.id)} aria-current={active ? 'page' : undefined}
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, background: active ? `${tab.color}08` : 'transparent', border: 'none', borderTop: `2px solid ${active ? tab.color : 'transparent'}`, cursor: 'pointer', padding: '4px 2px', minWidth: 0 }}
          >
            <span style={{ position: 'relative', fontSize: 15, color: active ? tab.color : C.textMuted, transition: 'color 0.15s' }}>
              {tab.icon}
              {sig !== 'idle' && tab.id !== 'chat' && (
                <span style={{ position: 'absolute', top: -2, right: -4, display: 'inline-block', width: 5, height: 5, borderRadius: '50%', background: sig === 'error' ? C.rose : sig === 'warning' ? C.amber : tab.color, animation: sig === 'processing' ? 'sdc-pulse 1s ease-in-out infinite' : 'none' }} />
              )}
            </span>
            <span style={{ fontFamily: 'monospace', fontSize: 7.5, color: active ? tab.color : C.textMuted, transition: 'color 0.15s', letterSpacing: 0.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>
              {tab.short}
            </span>
          </button>
        );
...
export function BuilderContainer({
  ...
  const [sequence, setSequence]     = useState<Array<{ tabId: string; auto: boolean }>>([]);
  const [signals, setSignals]       = useState<Record<string,SignalType>>(Object.fromEntries(MODULES.map(m => [m.id, 'idle' as SignalType])));
  const [phases, setPhases]         = useState<Record<string,AnimPhase>>(Object.fromEntries(MODULES.map(m => [m.id, 'idle' as AnimPhase])));
  const [conditions, setConditions] = useState<Partial<Record<ModuleId,ModuleCond[]>>>(INIT_CONDITIONS);
  ...
  const palStats = useMemo(() => {
    ...
    return { total: t, savings: Math.round(((t * 30 - cost) / (t * 30)) * 100) };
  }, [palDecisions]);
  ...
  useEffect(() => {
    ...
    const nextConditions: Partial<Record<ModuleId, ModuleCond[]>> = {
      ...
      logger: [
        ...
        { label: 'Runtime events recorded', status: statusLogs.length > 0 ? 'pass' : 'wait' },
      ],
    };
    ...
    for (const module of MODULES) {
      ...
      if (previous !== next) addLog('signal', `Signal[${module.id}] → ${next}`, module.id);
    }
  }, [
    addLog,
    agentDisabled,
  ...
  return (
    <section
      ...
      style={{ width: '100%', maxWidth: MAX_W, margin: '0 auto', height: '100dvh', display: 'flex', flexDirection: 'column', background: C.bg, color: C.text, fontFamily: 'system-ui, -apple-system, sans-serif', overflow: 'hidden', position: 'relative', WebkitTapHighlightColor: 'transparent' }}
    >
      <style>{`
      ...
      {isChat ? (
        ...
        <div style={{ flex: 1, overflowY: 'auto', background: C.bg }}>
          <ModuleScreen mod={activeMod} signals={signals} phases={phases} conditions={conditions} confidence={confidence} sequence={sequence} />
          <div style={{ height: 12 }} />
        </div>
      )}
