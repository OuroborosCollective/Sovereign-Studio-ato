...
function MessageCard({ message, onStartOrder, canStartOrder }: { message: ChatMessage; onStartOrder?: (text: string) => void; canStartOrder: boolean }) {
  const [actionsOpen, setActionsOpen] = useState(false);
  const human = message.sender === 'HUMAN' || message.role === 'human';
  const system = message.sender === 'SYSTEM' || message.role === 'system';
  const actionable = human || message.role === 'assistant' || message.sender === 'SOVEREIGN_SWARM';

  const handleStartOrder = () => {
    if (!actionable || !canStartOrder || !onStartOrder) return;
    setActionsOpen(false);
    onStartOrder(message.content);
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-2.5 ${human ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`relative max-w-[88%] sm:max-w-[82%] rounded-lg border px-3 py-2.5 ${human ? 'bg-[rgba(255,30,56,0.11)] border-[rgba(255,30,56,0.25)]' : 'bg-[var(--carbon-deep)] border-white/5'}`}>
        ...
        <div className="mt-2 flex items-center justify-between gap-3 font-mono text-[8.5px] text-[var(--text-dim)]">
          <span>{human ? 'OWNER' : system ? 'RUNTIME' : 'SOVEREIGN'}</span>
          <div className="flex items-center gap-1.5">
            {message.evidenceBadge && <span className="text-[var(--emerald-seal)] font-bold">{message.evidenceBadge}</span>}
            {actionable && (
              <div className="relative">
                <button type="button" aria-label="Nachrichtenaktionen" aria-expanded={actionsOpen} onClick={() => setActionsOpen((open) => !open)} className="min-h-10 min-w-10 rounded-md border border-white/10 bg-[var(--carbon-surface)] px-2 text-[var(--text-muted)] hover:text-white hover:border-[rgba(255,30,56,0.35)] flex items-center justify-center">
                  <MoreHorizontal size={14} />
                </button>
                {actionsOpen && (
                  <div role="menu" aria-label="Nachrichtenaktionen" className="absolute right-0 bottom-11 z-40 min-w-44 rounded-lg border border-white/10 bg-[var(--carbon-deep)] p-1.5 shadow-[0_12px_32px_rgba(0,0,0,0.55)]">
                    <button type="button" role="menuitem" onClick={handleStartOrder} disabled={!canStartOrder || !onStartOrder} title={canStartOrder ? 'Exakten Nachrichtentext direkt an Agent Zero senden' : 'Während eines laufenden Auftrags gesperrt'} className="w-full min-h-11 rounded-md px-3 py-2 text-left font-mono text-[10px] text-white hover:bg-white/5 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2">
                      <Rocket size={12} className="text-[var(--red-laser)]" /> Auftrag starten
                    </button>
                  </div>
                )}
              </div>
  ...
  return (
    <section className="flex flex-col h-full bg-[var(--carbon-base)] border-r border-[rgba(255,30,56,0.18)] relative overflow-hidden" data-testid="vnext-command-surface">
      <div className="px-3 sm:px-4 py-2.5 border-b border-white/5 bg-[var(--carbon-deep)] flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 min-w-0"><Terminal size={14} className="text-[var(--red-laser)] shrink-0" /><span className="font-mono text-[10px] sm:text-[11px] font-black tracking-widest text-white truncate">SOVEREIGN CHAT</span>{latestRun && <span className="hidden lg:inline font-mono text-[8.5px] text-[var(--text-dim)] truncate">RUN // {latestRun}</span>}</div>
      ...
      {chatError && <div role="alert" className="shrink-0 px-3 py-2 text-[11px] text-[var(--red-alert)] break-words">{chatError}</div>}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-4 space-y-3" data-testid="vnext-message-stream">{messages.map((message) => <MessageCard key={message.id} message={message} onStartOrder={onSubmitOrder} canStartOrder={!executing} />)}</div>
      ...
      <div className="shrink-0 px-3 sm:px-4 pb-3 sm:pb-4 pt-2 bg-gradient-to-t from-[var(--carbon-base)] via-[var(--carbon-base)] to-transparent">
        ...
        <div data-testid="advisory-chat-status" className="mb-2 rounded-lg border border-white/5 bg-[var(--carbon-deep)] p-2.5 font-mono">
          <div className="flex items-center gap-2"><Bot size={11} className="text-[var(--red-laser)]" /><span className="text-[8.5px] font-bold tracking-wider text-white">BERATUNGSMODUS</span><span className="text-[8px] text-[var(--text-dim)]">LLM spricht · UI startet Aufträge</span></div>
          <p className="mt-1 text-[8.5px] text-[var(--text-dim)]">Jede eigene oder Sovereign-Nachricht kann über ⋯ als exakter Auftrag an Agent Zero gesendet werden.</p>
        </div>
        ...
        <div className="theme-diamond-cut rounded-xl border border-[rgba(255,30,56,0.28)] bg-[var(--carbon-deep)] p-2 shadow-[0_0_24px_rgba(255,30,56,0.08)]">
          <textarea
            ...
            disabled={isChatBusy}
            placeholder={isChatBusy ? 'Sovereign formuliert gerade eine Antwort…' : 'Frag Sovereign etwas. Architektur, Repository, Umsetzungsideen oder fehlende Informationen – alles zuerst im Chat.'}
          ...
          />
          <div className="flex items-center justify-between gap-2 px-1 pt-1 border-t border-white/5"><div className="flex items-center gap-1 font-mono text-[8.5px] text-[var(--text-dim)]"><Cpu size={10} className="text-[var(--red-laser)]" /> ENTER sendet · SHIFT+ENTER neue Zeile</div><motion.button whileTap={{ scale: 0.96 }} type="button" data-testid="builder__start-task" onClick={submit} disabled={!canSend} className="min-h-9 px-3 rounded-md bg-[var(--red-pulse)] text-white font-mono text-[10px] font-black flex items-center gap-1.5 disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_0_12px_rgba(255,30,56,0.25)]"><Send size={11} /> SEND</motion.button></div>
        </div>
      </div>
    </section>
  );
