    ...
    <section className="flex flex-col h-full bg-[var(--carbon-base)] border-r border-[rgba(255,30,56,0.18)] relative overflow-hidden" data-testid="vnext-command-surface">
      ...
      <div className="shrink-0 px-3 sm:px-4 pb-3 sm:pb-4 pt-2 bg-gradient-to-t from-[var(--carbon-base)] via-[var(--carbon-base)] to-transparent">
        ...
        <div data-testid="agent-mode-selector" className="mb-2 flex items-center gap-1.5 rounded-lg border border-white/5 bg-[var(--carbon-deep)] p-1.5 font-mono">
          ...
          <button type="button" data-testid="agent-mode-single" aria-pressed={agentMode === 'single'} disabled={executing} onClick={() => { playKeystrokeChirp(); onAgentModeChange?.('single'); }} className={`min-h-8 flex-1 rounded border px-2 text-[8.5px] font-black ${agentMode === 'single' ? 'border-[var(--emerald-seal)] bg-[rgba(16,185,129,0.12)] text-[var(--emerald-seal)]' : 'border-white/5 bg-[var(--carbon-surface)] text-[var(--text-muted)]'} disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-[var(--emerald-seal)] focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--carbon-deep)]`}>1 AGENT · FREELLM</button>
          <button type="button" data-testid="agent-mode-swarm" aria-pressed={agentMode === 'swarm'} disabled={executing} onClick={() => { playKeystrokeChirp(); onAgentModeChange?.('swarm'); }} className={`min-h-8 flex-1 rounded border px-2 text-[8.5px] font-black ${agentMode === 'swarm' ? 'border-[var(--red-laser)] bg-[rgba(255,30,56,0.12)] text-[var(--red-laser)]' : 'border-white/5 bg-[var(--carbon-surface)] text-[var(--text-muted)]'} disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-[var(--red-laser)] focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--carbon-deep)]`}>SWARM · OPT-IN</button>
        ...
        <div className="theme-diamond-cut rounded-xl border border-[rgba(255,30,56,0.28)] bg-[var(--carbon-deep)] p-2 shadow-[0_0_24px_rgba(255,30,56,0.08)]">
          <textarea
            ...
            className="w-full min-h-[72px] max-h-36 resize-none bg-transparent outline-none px-2 py-1.5 font-mono text-[11px] text-white placeholder:text-[var(--text-dim)] disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-[var(--red-laser)] focus-visible:outline-none rounded-md"
          />
          <div className="flex items-center justify-between gap-2 px-1 pt-1 border-t border-white/5"><div className="flex items-center gap-1 font-mono text-[8.5px] text-[var(--text-dim)]"><Cpu size={10} className="text-[var(--red-laser)]" /> ENTER dispatches · SHIFT+ENTER newline</div><motion.button whileTap={{ scale: 0.96 }} type="button" data-testid="builder__start-task" onClick={submit} disabled={!canSend} className="min-h-9 px-3 rounded-md bg-[var(--red-pulse)] text-white font-mono text-[10px] font-black flex items-center gap-1.5 disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_0_12px_rgba(255,30,56,0.25)] focus-visible:ring-2 focus-visible:ring-[var(--red-laser)] focus-visible:outline-none focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--carbon-deep)]"><Send size={11} /> DISPATCH</motion.button></div>
        </div>
      </div>
    </section>
  );
