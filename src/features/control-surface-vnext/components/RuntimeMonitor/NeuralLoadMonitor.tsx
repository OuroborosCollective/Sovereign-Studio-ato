  ...
  return (
    <div className="bg-[var(--carbon-deep)] border-b border-[rgba(255,30,56,0.22)] p-2.5 font-mono select-none" data-testid="vnext-phase-activity">
      <div className="flex items-center justify-between mb-1.5 text-[10px]">
        ...
        <div className="flex items-center gap-2 text-[9px]"><span className="text-[var(--text-dim)]">EVENTS</span><span className="text-white font-bold">{evidenceCount}</span><span className="text-[var(--text-dim)]">CHANGED</span><span className="text-white font-bold">{files}</span></div>
      </div>
      <p className="text-[9px] text-[var(--text-muted)]">{active ? 'Showing state transitions only; repeated identical heartbeats are suppressed. Completion percentage is unavailable.' : 'Phase and counts come from the last received job state.'}</p>
