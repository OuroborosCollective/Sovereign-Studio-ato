...
export interface UseAdminFreeRevolverProvidersResult {
  ...
  autoConfigureKey: (apiKey: string, providerId?: string) => Promise<{ providerId: string; label: string; keyCount: number }>;
...
export function useAdminFreeRevolverProviders(): UseAdminFreeRevolverProvidersResult {
  ...
  const autoConfigureKey = useCallback(async (apiKey: string, providerId?: string) => {
    setError(null);
    if (apiKey.length < 8) throw new Error('Der API-Key muss mindestens 8 Zeichen enthalten.');
    const result = await adminApiClient.autoConfigureFreellmProviderKey(apiKey, providerId);
    reload();
