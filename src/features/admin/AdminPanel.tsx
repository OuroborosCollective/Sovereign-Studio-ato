/**
 * AdminPanel — Main admin UI with tab navigation.
 *
 * Tabs: Users | Billing | Launcher | LLM | Audit
 * Protected by AdminGate (role === 'admin' | 'superadmin').
...
import { useAdminUsers, useAdminTransactions, useAdminLauncherTools, useAdminLlmRoutes, useAdminAuditLog } from './hooks/useAdminApi';
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:20, padding:24, background:C.bg }}>
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8 }}>
        ...
        <div style={{ fontSize:14, fontWeight:700, color:C.text }}>Admin API-Key</div>
        <div style={{ fontSize:11, color:C.textSub, textAlign:'center', maxWidth:260 }}>
          Einmalig eingeben — wird in localStorage gespeichert.
          <br/>Ersetzt durch JWT-Auth nach Issue #459.
        </div>
      </div>

      <div style={{ width:'100%', maxWidth:280, display:'flex', flexDirection:'column', gap:10 }}>
        <input type="password" placeholder="8516ae6b…" value={input} onChange={e => setInput(e.target.value)}
        ...
        <button type="button" onClick={() => void handleSave()} disabled={testing||!input.trim()}
...
function PanelContent({ onClose, onMinimize }: LauncherToolProps) {
  ...
  const auditApi    = useAdminAuditLog();

  // Check if key already stored
  useEffect(() => {
    const key = getAdminKey();
    if (!key) return;
    adminApiClient.ping().then(() => setReady(true)).catch(() => setReady(false));
  }, []);
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', background:C.bg }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:14 }}>
        ...
        {tab==='billing'  && (<div style={{ display:'flex', flexDirection:'column', gap:16 }}><BillingStats/><TransactionTable api={txApi}/></div>)}
        {tab==='launcher' && <LauncherToolEditor api={launcherApi}/>}
        {tab==='llm'      && <LlmRouteEditor     api={llmApi}/>}
        {tab==='audit'    && <AuditLogView        api={auditApi}/>}
      </div>

      {editUser && <UserEditModal user={editUser} api={usersApi} onClose={() => { setEditUser(null); usersApi.reload(); }}/>}
    </div>
  );
}

// ── Audit log view (inline, small) ────────────────────────────────────────────

function AuditLogView({ api }: { api: ReturnType<typeof useAdminAuditLog> }) {
  const { entries, total, loading, error, reload } = api;
  return (
