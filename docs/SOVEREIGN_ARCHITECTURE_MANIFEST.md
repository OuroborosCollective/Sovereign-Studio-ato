...
8. **BLOCKIERT — KI-Coach-Integration-Workflow:** Der Push-Run `29666014826` endete als Workflow-Konfigurationsfehler ohne gestartete Jobs. PR #828 beseitigte am ersten Head den dynamischen Ausdruck `needs.$job.result`; GitHub erzeugte daraufhin fünf Primärjobs und legte als nächste erste Kante einen veralteten Symbolcheck auf `hasRealStopper` offen. Der kanonische Coach implementiert und verwendet `realStopper`; der aktuelle Arbeitsstand bindet die Prüfung exakt an diese Funktion. Erneute exakte PR-Head- und GitHub-Actions-Evidence stehen noch aus.
