...
**Baseline-Revision vor diesem Manifest-Update:** `afe3e7fb0415b7de9d201491655e85a7417e4aad`
...
**BELEGT:** Die Control Plane meldete `BROKER_READY`. PR #824 wurde vom vollständig grünen Head `7b15d7241597bd40a7f8228733ce5b58f3859545` als Squash-Commit `018e2eee49396a811c9a4539d652053d0d035047` übernommen. PR #825 wurde anschließend vom vollständig grünen Head `56564f206ac18a093cd2a101f9df1d6e1f446965` als Squash-Commit `afe3e7fb0415b7de9d201491655e85a7417e4aad` übernommen. Für die aktuelle Revision bestanden Validation, Veröffentlichung des immutable MCP-Images, Digestprüfung und VPS-Bootstrap. Der Live-Readback meldet dieselbe Revision, den immutable Image-Digest und die Image-ID `sha256:df2206b014e3a6ebf820d98c3a684858117a3e67d11e226fe827a7b5aa7e75f0`, den Archiv-Hash `9cb7117990bd72d399028d8185fe8fd976facf8809248c1503436d324cdf0ed2` sowie den Runtime-Evidence-Hash `92e97aad9a4c7472374c755d2a73b671c1ff2844324603ea221108574a675869`. Container, MCP-Protokoll, Broker-RPC, Host-Command-Worker und Python-/TypeScript-Cross-Runtime-Parität sind revisionsgebunden bereit; direkte eingehende Mutationen bleiben verboten.
...
**BLOCKIERT:** PR #825 wechselte die Schwesterzugriffe nach acht vollständig grünen PR-Workflows von den nicht vorhandenen Objektnamen auf Compose-Service-Aliase und wurde als Squash-Commit `afe3e7fb0415b7de9d201491655e85a7417e4aad` gemergt. Die offizielle Post-Merge-Kette veröffentlichte und installierte das immutable Image `sha256:df2206b014e3a6ebf820d98c3a684858117a3e67d11e226fe827a7b5aa7e75f0`; Revision, Image-ID, Archiv, Broker und MCP-Protokoll sind verifiziert. Der anschließend auf exakt dieser Runtime ausgeführte Fachcanary meldete trotzdem erneut `GOTENBERG_NETWORK_PEER_UNREACHABLE`. Damit ist die bloße Namenskorrektur als unzureichend widerlegt: Die getrennten Compose-Projekte teilen keine belegte DNS-/Netzwerkfläche.

**BELEGT — aktuelle Reproduktion und nächster kleinster Fix im Arbeitsstand:** Der Runtime-Readback bestätigt `gotenberg-acya-gotenberg-1` und `apache-tika-2l6t-tika-1` als laufende Compose-Services; `gpt-gotenberg` und `gpt-tika` existieren nicht. Gotenberg und Tika besitzen dynamische Hostportabbildungen, die weder hardcodiert noch als containerinterner Vertrag verwendet werden dürfen. Der neue bounded Host-Broker-Pfad löst deshalb jeweils genau einen laufenden Container über das exakte Compose-Service-Label auf, prüft Docker-State, Service-Identität und ausschließlich private Container-IP-Adressen und ruft die festen internen Ports `3000` und `9998` direkt vom Host auf. Mehrdeutigkeit, fehlende oder öffentliche Endpunkte, falsche Labels, Größenüberschreitungen und HTTP-/Netzwerkfehler blockieren fail-closed. PDF und Extraktion bleiben speicherlos und größenbegrenzt; der Pfad verwendet weder generische Shell, `docker exec`, veröffentlichte Hostports noch Netzwerkmutation. Zwölf gezielte Tests sowie die vollständige lokal ausführbare MCP-Suite mit `266 passed` und zwei ausschließlich sandboxbedingt abgewählten Unix-Socket-Tests sind grün. Der Pfad bleibt bis zu terminal grüner PR-, Merge-, immutable Deployment- und erfolgreicher Fachcanary-Evidence `BLOCKIERT`.
...
10. **BELEGT — MCP-Releasebasis:** Validation, immutable Image-Publish, Digestprüfung und VPS-Bootstrap bestanden für `afe3e7fb0415b7de9d201491655e85a7417e4aad`; der Live-Readback bestätigte dieselbe Revision und den immutable Digest `sha256:df2206b014e3a6ebf820d98c3a684858117a3e67d11e226fe827a7b5aa7e75f0`.
...
1. **BLOCKIERT — Dokumentpipeline:** Der Gotenberg→Tika-Livecanary meldete auch nach vollständig grünem PR #825, SHA-gebundenem Merge, immutable Deployment und revisionsverifiziertem Readback auf `afe3e7fb0415b7de9d201491655e85a7417e4aad` weiterhin `GOTENBERG_NETWORK_PEER_UNREACHABLE`. Die Namenskorrektur allein war damit unzureichend. Der aktuelle kleinste Fix entdeckt die getrennten Compose-Services fail-closed über Labels und verifizierte private Container-Endpunkte direkt vom Host-Broker; sein Produktionsbeweis steht bis zu neuer PR-, Merge-, Deployment- und Live-Canary-Evidence aus.
...
**BLOCKIERT:** Ein vollständiger Produkt- und Release-Grünstatus wird nicht behauptet. A2A, Backend-Ownership, Endpoint-Referenz, Knowledge-Integrität, signierte Android-Releaseartefakte und die produktive Enterprise-MCP-Werkzeugkette sind belegt. Aktuelle harte Blocker sind der auch nach immutable Deployment von Revision `afe3e7fb0415b7de9d201491655e85a7417e4aad` mit `GOTENBERG_NETWORK_PEER_UNREACHABLE` fehlgeschlagene Dokumentcanary, der joblose KI-Coach-Workflow-Konfigurationsfehler `29666014826` sowie die aktuell fehlschlagenden echten LiteLLM-Completion-Canaries. Der geschützte Providerdiagnosepfad liefert für den aktuellen Modellversuch keine belastbar feinere Ursache; eine Kontingent-, Credential- oder andere Providerdiagnose wird daher nicht als heutige Wahrheit behauptet. Zusätzlich fehlen der physische Android-Geräte-Smoke, der produktive Action-Stream-Doppel-Append-Canary und ein erreichbares optionales Milvus-Backend.

---

# 26. Aktueller Evidence-Snapshot

Evidence-Stand: 19. Juli 2026. PR #820 und die GitHub-Knowledge-Transportänderung aus PR #821 sind in der aktuellen Repository-Baseline enthalten. PR #824 brachte die Enterprise-Backend-/Revisionswerkzeuge über die offizielle immutable MCP-Releasekette produktiv online. PR #825 war an Head `56564f206ac18a093cd2a101f9df1d6e1f446965` mit acht grünen Workflows gebunden und wurde als Squash-Commit `afe3e7fb0415b7de9d201491655e85a7417e4aad` gemergt und immutable deployt. Der Fachcanary widerlegte anschließend die Annahme, dass Service-Aliase über die getrennten Compose-Netze erreichbar seien. Der daraus folgende bounded Host-Broker-Fix besitzt lokale Code- und Test-Evidence, ist aber noch nicht gemergt oder deployt.

- **BELEGT:** Repository-Baseline für diese Folgearbeit ist der neu aufgelöste, beim Start mit `origin/main` identische Commit `afe3e7fb0415b7de9d201491655e85a7417e4aad`.
- **BELEGT:** Für MCP-Release-Revision `afe3e7fb0415b7de9d201491655e85a7417e4aad` bestanden MCP-Validation, immutable Image-Publish, Digestprüfung und VPS-Bootstrap. Der Live-Readback bestätigte Revision, Image-Revision und Image-ID mit Digest `sha256:df2206b014e3a6ebf820d98c3a684858117a3e67d11e226fe827a7b5aa7e75f0`; direkte eingehende Mutationen bleiben verboten.
...
- **BLOCKIERT:** Der Dokumentcanary meldete nach dem vollständig grünen, gemergten und auf `afe3e7fb0415b7de9d201491655e85a7417e4aad` immutable deployten Alias-Fix erneut `GOTENBERG_NETWORK_PEER_UNREACHABLE`. Der aktuelle bounded Host-Broker-Fix löst exakt einen laufenden Gotenberg-/Tika-Service über Compose-Labels und private Docker-Endpunkte auf, ohne `docker exec`, Hostportabhängigkeit oder Netzwerkmutation. Zwölf gezielte Tests und die lokal ausführbare Gesamtsuite mit `266 passed, 2 deselected` sind grün; PR- und Livebeweis stehen aus.
