...
**Zusätzlicher Produktentwicklungsstrang, nicht Teil des historischen Vier-Issue-Open-Point-Bundles:** **TEILWEISE — Zero-Trust Repair Capsule, Issues #1122 bis #1125.** Der reine Core aus Issue #1123 bindet Patchbytes, Basisrevision, Outcome Contract, abgeleitete Pfade, Test-Evidence und einen unabhängigen Offline-Verifier deterministisch und fail-closed. Eine tenantgebundene Downloadroute und UI existieren damit noch nicht; sie bleiben Issue #1124. Reale Zero-Write-Abnahme und beobachtete Benchmarks bleiben Issue #1125. Deshalb wird weder Produktverfügbarkeit noch Runtime-Erfolg behauptet.
...
**BLOCKIERT/TEILWEISE:** Ein vollständiger Produkt- und Release-Grünstatus wird nicht behauptet. Migration `028/041`, Standing-Owner-Policy, beide Mirrorfamilien, GitHub-App-Routenregistrierung, die einzige Admin-UI-Wahrheit und die direkte OpenRouter-/FreeLLM-Transportarchitektur sind reconciliiert. OpenRouter benötigt aktuell einen Katalog-Refresh und besitzt `0` auswählbare Modelle; FreeLLM besitzt fünf Ready-Routen, bleibt als Gesamtquelle aber `degraded`. Releasekritisch offen bleiben ausschließlich die vier operativen Punkte aus Abschnitt 24.2: physischer Android-Smoke (#1013), GitHub-Knowledge-Livecanary (#1014), Filebrowser-Entscheidung (#1016) sowie Host-Patch/Reboot (#1017). Der zusätzliche Zero-Trust-Capsule-Produktstrang besitzt derzeit nur den reinen Core; Delivery/UI (#1124) und reale Zero-Write-Abnahme (#1125) fehlen. Der Core verändert den globalen Releasestatus nicht. Ein globaler Grünstatus bleibt bis zum getrennten, revisionsgebundenen Abschluss der jeweiligen Pflichtbelege unzulässig.
...
- **Zero-Trust Repair Capsule, Code-Evidence:** Der Commit, der diesen Manifeststand enthält, führt den reinen `sovereign.repair-capsule.v1`-Core, bytegleiche Analyse-/Deploymentspiegel, einen unabhängigen Offline-Verifier und adversariale Vertragstests. Der Core führt keine Netzwerk-, GitHub-Schreib-, Apply-, API- oder UI-Wirkung aus. Delivery und reale Produktabnahme bleiben ausdrücklich offen; PR-Head, Merge-SHA und CI-Gates werden nur über GitHub-Readback gebunden.
