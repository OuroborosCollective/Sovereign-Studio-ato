...
**Baseline-Revision vor diesem Manifest-Update:** `e2442e4f3c63a1638aa30b7a4339e6a83c2a7f6b`
...
**BLOCKIERT — aktuelle erste gebrochene Kante:** PR #826 führte nach acht vollständig grünen PR-Workflows den bounded Host-Broker-Pfad ein und wurde als Squash-Commit `e2442e4f3c63a1638aa30b7a4339e6a83c2a7f6b` gemergt. Die offizielle Post-Merge-Kette installierte das immutable Image `sha256:b33c91968259208647b5b389cc1a75eac06b9f4dc354d2afa36cc3af953cccca`; Revision, Image-ID, Archiv, Broker und MCP-Protokoll sind verifiziert. Der Fachcanary erreicht Gotenberg jetzt nachweislich über dessen private Containeradresse. Statt des bisherigen Netzwerkfehlers liefert der reale Konversionsendpoint nun `GOTENBERG_CONVERSION_HTTP_401`. Bounded Gotenberg-Logs korrelieren denselben POST-Pfad mit `401 Unauthorized`. Netzwerk und Routing sind damit repariert; die erste verbleibende Kante ist die aktivierte Gotenberg-Basic-Auth.

**BELEGT — nächster kleinster Fix im Arbeitsstand:** Gotenberg 8 aktiviert Basic Auth laut offiziellem Vertrag über `API_ENABLE_BASIC_AUTH` und erwartet dann `GOTENBERG_API_BASIC_AUTH_USERNAME` und `GOTENBERG_API_BASIC_AUTH_PASSWORD`. Der Host-Broker wertet aus dem bereits service- und containerverifizierten Gotenberg-Objekt ausschließlich diese beiden benannten Werte für den flüchtigen Authorization-Header aus. Er gibt weder Werte, Header noch übrige Container-Umgebung aus oder weiter, persistiert sie nicht und löscht die lokale Headerstruktur nach dem Request. Fehlende, nur teilweise vorhandene, übergroße oder kontrollzeichenhaltige Werte blockieren fail-closed. Der bestehende Pfad bleibt speicherlos und größenbegrenzt und verwendet weiterhin keine generische Shell, veröffentlichte Hostports oder Netzwerkmutation. Vierzehn gezielte Tests sowie die vollständige lokal ausführbare MCP-Suite mit `268 passed` und zwei ausschließlich sandboxbedingt abgewählten Unix-Socket-Tests sind grün. Der Auth-Pfad bleibt bis zu terminal grüner PR-, Merge-, immutable Deployment- und erfolgreicher Fachcanary-Evidence `BLOCKIERT`.
...
10. **BELEGT — MCP-Releasebasis:** Validation, immutable Image-Publish, Digestprüfung und VPS-Bootstrap bestanden für `e2442e4f3c63a1638aa30b7a4339e6a83c2a7f6b`; der Live-Readback bestätigte dieselbe Revision und den immutable Digest `sha256:b33c91968259208647b5b389cc1a75eac06b9f4dc354d2afa36cc3af953cccca`.
...
1. **BLOCKIERT — Dokumentpipeline:** Der Host-Broker-Fix aus PR #826 ist vollständig grün, gemergt und als Revision `e2442e4f3c63a1638aa30b7a4339e6a83c2a7f6b` immutable deployt. Der Livecanary erreicht Gotenberg jetzt, erhält jedoch `GOTENBERG_CONVERSION_HTTP_401`; die Logs bestätigen denselben unauthentisierten POST. Der aktuelle kleinste Fix übernimmt die bereits serverseitig vorhandene Gotenberg-Basic-Auth ausschließlich flüchtig und redaktionssicher in den Conversion-Request. Sein Produktionsbeweis steht bis zu neuer PR-, Merge-, Deployment- und Live-Canary-Evidence aus.
...
**BLOCKIERT:** Ein vollständiger Produkt- und Release-Grünstatus wird nicht behauptet. A2A, Backend-Ownership, Endpoint-Referenz, Knowledge-Integrität, signierte Android-Releaseartefakte und die produktive Enterprise-MCP-Werkzeugkette sind belegt. Aktuelle harte Blocker sind der auf Revision `e2442e4f3c63a1638aa30b7a4339e6a83c2a7f6b` bis zum realen Gotenberg-Endpoint fortgeschrittene, dort aber mit `GOTENBERG_CONVERSION_HTTP_401` blockierte Dokumentcanary, der joblose KI-Coach-Workflow-Konfigurationsfehler `29666014826` sowie die aktuell fehlschlagenden echten LiteLLM-Completion-Canaries. Der geschützte Providerdiagnosepfad liefert für den aktuellen Modellversuch keine belastbar feinere Ursache; eine Kontingent-, Credential- oder andere Providerdiagnose wird daher nicht als heutige Wahrheit behauptet. Zusätzlich fehlen der physische Android-Geräte-Smoke, der produktive Action-Stream-Doppel-Append-Canary und ein erreichbares optionales Milvus-Backend.

---

# 26. Aktueller Evidence-Snapshot

Evidence-Stand: 19. Juli 2026. PR #820 und die GitHub-Knowledge-Transportänderung aus PR #821 sind in der aktuellen Repository-Baseline enthalten. PR #824 brachte die Enterprise-Backend-/Revisionswerkzeuge über die offizielle immutable MCP-Releasekette produktiv online. PR #825 widerlegte nach vollständig grünem Merge und Deployment die netzübergreifende Aliasannahme. PR #826 war am Head `a5d3fecdb446232149e199c1b2270092bffe0a3a` mit acht grünen Workflows gebunden, wurde als Squash-Commit `e2442e4f3c63a1638aa30b7a4339e6a83c2a7f6b` gemergt und immutable deployt und reparierte die private Service-Erreichbarkeit. Der Fachcanary erreichte danach Gotenberg und legte dessen Basic-Auth-401 als nächste erste gebrochene Kante frei. Der Auth-Fix besitzt lokale Code- und Test-Evidence, ist aber noch nicht gemergt oder deployt.

- **BELEGT:** Repository-Baseline für diese Folgearbeit ist der neu aufgelöste Main- und Merge-Commit `e2442e4f3c63a1638aa30b7a4339e6a83c2a7f6b`.
- **BELEGT:** Für MCP-Release-Revision `e2442e4f3c63a1638aa30b7a4339e6a83c2a7f6b` bestanden MCP-Validation, immutable Image-Publish, Digestprüfung und VPS-Bootstrap. Der Live-Readback bestätigte Revision, Image-Revision und Image-ID mit Digest `sha256:b33c91968259208647b5b389cc1a75eac06b9f4dc354d2afa36cc3af953cccca`; direkte eingehende Mutationen bleiben verboten.
...
- **BLOCKIERT:** Der Dokumentcanary erreicht auf dem vollständig grünen, gemergten und immutable deployten Host-Broker-Fix Gotenberg über dessen private Containeradresse, erhält aber `GOTENBERG_CONVERSION_HTTP_401`; die bounded Logs bestätigen den unauthentisierten POST. Der aktuelle Fix verwendet ausschließlich die zwei benannten Gotenberg-Basic-Auth-Werte für einen flüchtigen, nie ausgegebenen Header. Vierzehn gezielte Tests und die lokal ausführbare Gesamtsuite mit `268 passed, 2 deselected` sind grün; PR- und Livebeweis stehen aus.
