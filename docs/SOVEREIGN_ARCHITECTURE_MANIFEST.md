...
**BELEGT:** Die Control Plane meldet einen gesunden privaten MCP auf Revision `7ef2ad6cefe0659b240be155a64c044df088ac08` und immutable Digest `sha256:64b4290dce3450b007bade472b1d9998875645d1873fe514833f7778ef9cf91a`. Die vollständige CI-Suite bestand mit `465 passed, 12 skipped`; Image-Publish und Digest-Verify waren erfolgreich. Der Self-Update-, PatchMon-, Container- und Broker-Readback bestätigt Healthy-State, Host-Command-Worker, Broker-RPC, MCP-Protokoll `2025-06-18`, Cross-Runtime-Parität und exakte Revision. Der deterministische Konfigurations-Evidence-Hash lautet `d3da0b886d5cc53f84ec2316e3d14073ae70b9022775e8c0602b8369cf50ffc0`. Repository-, Backend- und MCP-Revision werden bewusst getrennt ausgewiesen. Direkte eingehende Mutationen, generische Shell und ein Docker-Socket im MCP bleiben verboten.
...
**BELEGT — historische Releasekette und aktueller Runtimepfad:** Dockerfile, Launcher, Installer, Releasearchiv-Check und GitHub-Workflow sind verdrahtet. PR #854 und PR #867 bilden historische revisionsgebundene Release-Provenance. Die aktuell installierte MCP-Revision, ihr Digest und der aktuelle Runtime-Evidence-Hash werden ausschließlich aus Abschnitt 9.2 und 26.1 abgeleitet; ältere Digests in historischen Absätzen besitzen keine gegenwärtige Statusautorität.
...
10. **BELEGT — MCP-Releasebasis:** PR #854 bestand am unveränderten Head `380d1c34a1a8205e90d7a6fe53181128ee29f75b` alle 18 Checkläufe ohne Fehler und wurde als Squash-Commit `11016a246af0a1f866c1814178f4fed7f82746c8` übernommen. Diese Revision bleibt historische Release-Provenance; der aktuelle immutable MCP-Stand wird in Abschnitt 9.2 und 26.1 revisions- und digestgebunden ausgewiesen.
...
22. **BELEGT — PR #867 Code- und MCP-Integration, historisch:** PR #867 wurde nach vollständig grünen Gates als Squash-Commit `246e701f5b2d8ce94c21c001b26bf4aee216239c` übernommen. Der damalige immutable MCP-Stand bleibt Provenance; Enterprise-Admin-UI, Standing-Learning-Policy, Backend-Deployment und Migration `028/041` wurden inzwischen durch spätere revisionsgebundene Evidence ersetzt beziehungsweise abgeschlossen.
...
33. **BELEGT — Manifest-/Open-Point-Reconciliation / Issue #1011:** PR #1025 übernahm die fachliche Reconciliation, PR #1026 trennte Repository- und Runtime-Revisionsautorität ohne rekursive SHA-Behauptung, und PR #1027 löste den widersprüchlichen Review-Evidence-Testvertrag, ohne einen dritten direkten Required Runner einzuführen. Der aktuelle MCP-Release-Commit ist `7ef2ad6cefe0659b240be155a64c044df088ac08`. Das Bundle bindet ausschließlich die vier tatsächlich offenen GitHub-Issues #1013, #1014, #1016 und #1017. Der Workflow validiert vorhandene Issue-Nummern, verweigert Titelabweichungen und öffnet geschlossene Punkte nicht automatisch erneut.
...
**Zusätzlicher Produktentwicklungsstrang, nicht Teil des historischen Vier-Issue-Open-Point-Bundles:** **TEILWEISE — Zero-Trust Repair Capsule, Issues #1122 bis #1125.** Core und Delivery-Code binden Patchbytes, Basisrevision, Outcome Contract, serverseitigen Workspace-Diff, abgeleitete Pfade, Test-Evidence und einen unabhängigen Offline-Verifier deterministisch und fail-closed. Die tenantgebundene Route akzeptiert keine Patch- oder GitHub-Credential-Felder und die UI trennt Draft PR und Capsule ausdrücklich. Produktverfügbarkeit bleibt bis exact-head CI, Merge, immutable Deployment und echter Zero-Write-Abnahme unbehauptet; die reale Sandbox-Abnahme und beobachtete Benchmarks bleiben Issue #1125. Für #1125 ist mit `backend/agent_runtime/repair_capsule_benchmark.py` (bytegleicher Deployment-Spiegel, Backend-Vertragstests) die deterministische Evidence-Harness `IMPLEMENTED_IN_REPOSITORY`: sie prüft statische Zero-Write-Invarianten netzwerkfrei, führt gegen einen realen Fresh-Clone `verify.py`, `git apply --check`, lokalen Apply, gezielte Testparität und Hash-Readback durch, rollt den Apply zurück und notiert nicht beobachtbare Werte als `not measured`. Die reale Abnahme mit beobachteten Benchmark-Werten gegen vier owner-freigegebene Sandbox-Repos bleibt ausstehend und darf nicht durch ein statisches Manifest ersetzt werden.
...
**BLOCKIERT/TEILWEISE:** Ein vollständiger Produkt- und Release-Grünstatus wird nicht behauptet. Migration `028/041`, Standing-Owner-Policy, beide Mirrorfamilien, GitHub-App-Routenregistrierung, die einzige Admin-UI-Wahrheit und die direkte OpenRouter-/FreeLLM-Transportarchitektur sind reconciliiert. OpenRouter benötigt aktuell einen Katalog-Refresh und besitzt `0` auswählbare Modelle; FreeLLM-Ready-Routen bleiben ausschließlich runtime- und revisionsgebundene Evidence; dieses statische Manifest behauptet keine feste Anzahl und akzeptiert einen `degraded`-Readback nicht als gesund. Releasekritisch offen bleiben ausschließlich die vier operativen Punkte aus Abschnitt 24.2: physischer Android-Smoke (#1013), GitHub-Knowledge-Livecanary (#1014), Filebrowser-Entscheidung (#1016) sowie Host-Patch/Reboot (#1017). Der zusätzliche Zero-Trust-Capsule-Produktstrang besitzt Core und Delivery/UI-Code; exact-head Release-/Runtime-Evidence für #1124 sowie die reale Zero-Write-Abnahme #1125 bleiben offen. Der Core verändert den globalen Releasestatus nicht. Ein globaler Grünstatus bleibt bis zum getrennten, revisionsgebundenen Abschluss der jeweiligen Pflichtbelege unzulässig.
...
- **Zero-Trust Repair Capsule, Code-Evidence:** Der Commit, der diesen Manifeststand enthält, führt den `sovereign.repair-capsule.v1`-Core, bytegleiche Produktionsspiegel, einen unabhängigen Offline-Verifier sowie eine tenant-/CSRF-gebundene, größenbegrenzte Attachment-Route und die explizite Draft-PR/Capsule-UI-Auswahl. Die Route liest den isolierten Workspace und akzeptiert keine Patch- oder GitHub-Credential-Felder. Weder Code-Evidence noch ein Download beweisen Anwendung, Push, Merge, Deployment oder gesunde Produktion; PR-Head, Merge-SHA, CI, immutable Digest und Runtime bleiben getrennte Readbacks.
- **MCP:** Revision `7ef2ad6cefe0659b240be155a64c044df088ac08`, Digest `sha256:64b4290dce3450b007bade472b1d9998875645d1873fe514833f7778ef9cf91a`, Konfigurations-/Runtime-Evidence `d3da0b886d5cc53f84ec2316e3d14073ae70b9022775e8c0602b8369cf50ffc0`; `465 passed, 12 skipped`, Image-Publish, Digest-Verify, Self-Update, Healthy-Container, Broker-RPC, MCP-Protokoll `2025-06-18` und Cross-Runtime-Parität sind belegt.
