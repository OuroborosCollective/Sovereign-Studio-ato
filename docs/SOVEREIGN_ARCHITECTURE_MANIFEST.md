...
**BELEGT — Reproduktion und kleinster Fix im aktuellen Arbeitsstand:** Der aktuelle Runtime-Readback bestätigt `gpt-browserless`, `gotenberg-acya-gotenberg-1` und `apache-tika-2l6t-tika-1` als laufend, nicht pausiert, nicht restartend und ohne OOM-/Exitfehler. Die früher erwarteten Objekte `gpt-gotenberg` und `gpt-tika` existieren dagegen nicht mehr. Gotenberg veröffentlicht aktuell `32783:3000`, Tika `32785:9998`; diese zufälligen Hostports dürfen nicht in den containerinternen Fachcanary einfließen. Der Browserless-Anker gehört zum Compose-Projekt `gpt-tools` und zum Netzwerk `mcp-proxy`. Der Canary läuft als fester, speicherloser Node-Prozess in diesem Peer. Sein Code verwendete für die Schwesterzugriffe standardmäßig die nicht vorhandenen Container-Namen `gpt-gotenberg` und `gpt-tika`, statt die stabilen internen Service-Aliase `gotenberg` und `tika` mit ihren Serviceports zu verwenden. Der kleinste kausale Codefix wechselt ausschließlich diese beiden Peer-Defaults auf die Service-Aliase und behält `gpt-browserless` als Docker-Exec-Anker bei. Elf gezielte Dokumentpipeline-Tests sowie die vollständige lokal ausführbare MCP-Suite mit `265 passed` und zwei ausschließlich sandboxbedingt abgewählten Unix-Socket-Tests sind grün. Der Livecanary reproduziert am bisherigen Deployment weiterhin `GOTENBERG_NETWORK_PEER_UNREACHABLE`. Der Fix bleibt bis zu terminal grüner PR-, Merge-, immutable Deployment- und erneutem Live-Canary-Evidence `BLOCKIERT`; erst der erfolgreiche Runtime-Canary bestätigt die netzwerkseitige Erreichbarkeit der Service-Aliase vollständig.
