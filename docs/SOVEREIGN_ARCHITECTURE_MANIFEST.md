...
8. **BLOCKIERT — KI-Coach-Integration-Workflow:** Der Push-Run `29666014826` endete als Workflow-Konfigurationsfehler ohne gestartete Jobs. Die erste gebrochene Kante war der dynamische Ausdruck `needs.$job.result`. Der aktuelle Arbeitsstand ersetzt ihn durch vier statisch adressierbare `needs.<job>.result`-Prüfungen und ergänzt einen zentralen Regression-Guard; exakte PR-Head- und GitHub-Actions-Evidence stehen noch aus.
...
- **BLOCKIERT:** Der unabhängige Push-Run `29666014826` des KI-Coach-Integrationsworkflows scheiterte vor der Joberzeugung an seiner Workflowkonfiguration. Der Arbeitsstand ersetzt den unauflösbaren dynamischen `needs`-Zugriff durch vier explizite Resultatprüfungen und einen Regression-Guard; PR- und CI-Beweis stehen aus.
