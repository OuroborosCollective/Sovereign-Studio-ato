...
3. **All evidence digests are canonical SHA-256; missing or malformed hashes block the channel**
4. **Runtime revisions use the full 40-character Git SHA**
5. **Conflicting channels are visible in bundle**
6. **Unknown/unkalibrated scores trigger no action**
7. **Changed revision/config/weights invalidate old predictions**

## Files Changed

```
src/predictive/inference/
├── hash.ts                          # Canonical browser-safe SHA-256
├── hash.test.ts                     # Published SHA-256 test vectors
├── types.ts                         # Core types
...
├── riskEvidenceBundle.test.ts       # Bundle integrity/fail-closed tests
...
| Canonical SHA-256 evidence hashing | ✅ IN_REPOSITORY |
