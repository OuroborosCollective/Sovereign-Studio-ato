    ...
    def dispatch(
        ...
        *,
        ...
        handlers = {
            ...
            "aurion_genkit_status": lambda values: self.aurion.genkit_status(
                expected_revision=str(values.get("expected_revision") or ""),
            ),
            "aurion_genkit_propose": lambda values: self.aurion.genkit_propose(
                intent=str(values.get("intent") or ""),
                expected_revision=str(values.get("expected_revision") or ""),
                context=values.get("context") if isinstance(values.get("context"), dict) else {},
            ),
            "aurion_genkit_apply_plan": lambda values: self.aurion.genkit_apply_plan(
                proposal_sha256=str(values.get("proposal_sha256") or ""),
                expected_revision=str(values.get("expected_revision") or ""),
            ),
            "aurion_genkit_apply": lambda values: self.aurion.genkit_apply(
                proposal_sha256=str(values.get("proposal_sha256") or ""),
                expected_revision=str(values.get("expected_revision") or ""),
                confirmation_sha256=str(values.get("confirmation_sha256") or ""),
                owner_approved=standing_owner_delegation_approved(
