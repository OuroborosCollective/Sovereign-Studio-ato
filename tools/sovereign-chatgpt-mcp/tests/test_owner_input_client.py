...
def test_create_request_sends_fixed_target_and_no_protected_value(monkeypatch) -> None:
    ...
    result = client.create_openhands_request(
        title="OpenHands benötigt Zugang",
        reason="Der bestätigte Serverlauf kann ohne Provider-Zugang nicht fortgesetzt werden.",
    )
    ...
    assert call["json"]["targetId"] == "openhands_api_key"
...
def test_client_requires_private_bridge_key(monkeypatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="nicht konfiguriert"):
        client.create_openhands_request(title="Titel", reason="Begründung")
