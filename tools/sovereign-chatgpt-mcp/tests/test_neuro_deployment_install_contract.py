...
def test_installer_binds_revision_policy_permissions_and_preserves_predecessor_surface(
    ...
    assert 'EXPECTED_MCP_TOOL_COUNT="257"' in script.split('INSTALL_STAGE="configure_private_owner_mode"', 1)[1]
...
""",
    ...
    assert current_registry["toolCount"] == 257
    ...
    predecessor_registry["tools"] = [
        ...
        if item["name"] not in EXPECTED_NEURO_TOOLS | EXPECTED_AURION_TOOLS | EXPECTED_AGENT_ZERO_TOOLS | {"n8n_workflow_plan", "n8n_workflow_apply", "wolfram_cag_runtime_evidence_bind"}
    ]
    ...
    assert set(compatibility_receipt["additions"]) == EXPECTED_NEURO_TOOLS | EXPECTED_AURION_TOOLS | EXPECTED_AGENT_ZERO_TOOLS | {"wolfram_cag_runtime_evidence_bind"}
...
def test_installer_runs_a_clean_real_registry_neuro_canary_without_selected_tool_execution() -> None:
    ...
    assert 'assert len(set(guarded_tool_names)) == 252' in canary
