...
SAFE_TOKEN_FIXTURE = "owner-github-token-fixture-0123456789abcdef"
...
def test_owner_managed_github_token_file_is_used_only_as_protected_fallback(
    ...
    token_file.write_text(SAFE_TOKEN_FIXTURE, "utf-8")
    ...
    with auth.token() as issued:
        assert issued == SAFE_TOKEN_FIXTURE
    with auth.headers() as headers:
        assert headers["Authorization"] == f"Bearer {SAFE_TOKEN_FIXTURE}"
