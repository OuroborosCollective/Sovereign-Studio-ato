...
[[ -f "$SOURCE_DIR/continuity-data/SOVOTTT_PERSONAL_MEMORY.md" ]] || fail "runtime personal memory journal is missing"
...
install -d -m 0750 "$INSTALL_ROOT" "$BIN_DIR" "$BROKER_DIR" "$COMPOSE_TEMPLATE_ROOT" "$PGBACKWEB_TEMPLATE_DIR" "$PATCHMON_TEMPLATE_DIR" "$CODE_SERVER_TEMPLATE_DIR" "$MILVUS_TEMPLATE_DIR" "$FREELLMAPI_TEMPLATE_DIR" "$FREELLMPOOL_TEMPLATE_DIR" "$INSTALL_ROOT/continuity-data"
...
for file in Dockerfile requirements.txt policy.py runtime.py database.py database_evidence_tools.py command_contract.py command_queue.py broker_client.py owner_input_client.py a2a_runtime_client.py document_pipeline.py github_knowledge_canary.py programming_language_catalog_runtime.py github_issue_contracts.py owner_input_widget.py self_heal.py android_hardening.py android_validation_router.py mcp_protocol_health.py sovereign_cognitive_widget.py server.py tool_extensions.py llm_boundary_contract.py repository_skill_tools.py proven_learning_tools.py skill_supply_chain_tools.py deterministic_contract.py deterministic_architecture_tools.py enterprise_backend_tools.py freemium_product_architect_tools.py openai_project_access_tools.py continuity.py validate_continuity.py operating_profile.py operational_governance_tools.py operational_assurance_tools.py output_contracts.py toolchain_composition.py patchmon_operator.py patchmon_fleet.py launcher.py docker-compose.yml; do
  backup_control_plane_file "$INSTALL_ROOT/$file"
  install -m 0644 "$SOURCE_DIR/$file" "$INSTALL_ROOT/$file"
done
install -m 0644 "$SOURCE_DIR/continuity-data/SOVOTTT_PERSONAL_MEMORY.md" "$INSTALL_ROOT/continuity-data/SOVOTTT_PERSONAL_MEMORY.md"
...
docker exec sovereign-chatgpt-mcp test -f /app/continuity-data/SOVOTTT_PERSONAL_MEMORY.md || fail "personal memory journal is missing from the MCP image"
...
assert len(continuity_read.personalMemorySha256) == 64, continuity_read
assert "# Sovottt Personal Memory" in continuity_read.personalMemory, continuity_read
    ...
    json.dumps(
        {
            ...
            "personalMemorySha256": continuity_read.personalMemorySha256,
