...
verify_host_worker_canary() {
  local attempt canary_result queue_counts
  canary_result=""
  for attempt in $(seq 1 12); do
    if canary_result="$(docker exec -i sovereign-chatgpt-mcp python - <<'PY'
import json
import server

result = server.broker.call("host_worker_canary", {}, timeout=5)
print(json.dumps(result, sort_keys=True, separators=(",", ":")))
if result.get("status") != "HOST_WORKER_READY":
    raise SystemExit(1)
if result.get("execution_origin") != "host_worker":
    raise SystemExit(1)
PY
)"; then
      printf '%s\n' "$canary_result"
    ...
    if ! systemctl is-active --quiet sovereign-chatgpt-command-worker.service; then
      systemctl status sovereign-chatgpt-command-worker.service --no-pager >&2 || true
      fail "host command worker left the active state before the canary succeeded"
    fi
    sleep 2
  done
  queue_counts="$(python3 - "$COMMAND_QUEUE_DIR" <<'PY'
from pathlib import Path
import json
import sys
...
print(json.dumps({
    name: len([item for item in (root / name).glob("*.json") if item.is_file()])
    for name in ("inbox", "processing", "outbox")
}, sort_keys=True, separators=(",", ":")))
PY
)"
  printf 'last_host_worker_canary=%s\n' "${canary_result:-missing}" >&2
  printf 'host_command_queue_counts=%s\n' "$queue_counts" >&2
  fail "host worker canary did not become ready after bounded retries"
}
