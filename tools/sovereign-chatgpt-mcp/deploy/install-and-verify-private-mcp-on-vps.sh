...
assert len(tool_names) == 254, len(tool_names)
assert expected_tools <= tool_names, sorted(expected_tools - tool_names)
assert len(tool_names - expected_tools) == 249, len(tool_names - expected_tools)
...
assert status.evidence['toolCount'] == 254, status
