...
assert len(tool_names) == 250, len(tool_names)
assert expected_tools <= tool_names, sorted(expected_tools - tool_names)
assert len(tool_names - expected_tools) == 245, len(tool_names - expected_tools)
...
assert status.evidence['toolCount'] == 250, status
