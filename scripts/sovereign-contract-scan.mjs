...
function run() {
  ...
  forbidText('src/App.tsx', /PlayReleaseChat|data-layout="chat-only-live-entry"/, 'app:no-release-chat-root', 'The authenticated root must not regress to the retired chat-only release surface.');
  requireText('src/features/product/containers/BuilderContainer.tsx', /fetchSovereignDirectLlmInterpretation/, 'builder:llm-action-boundary', 'Builder uses the online structured LLM contract as the repository-action boundary.');
