...
function run() {
  ...
  requireText('src/features/release/PlayReleaseChat.tsx', /fetchSovereignDirectLlmInterpretation/, 'play-chat:llm-action-boundary', 'Play release chat uses the online LLM intent schema as the primary repository action boundary.');
  requireText('src/features/release/PlayReleaseChat.tsx', /if \(interpreted\.ok && interpreted\.interpretation\)[\s\S]*deriveRepositoryActionFallback\(text\)[\s\S]*Modellprosa wurde verworfen/, 'play-chat:degraded-action-fallback', 'Only a failed online action contract may enter the bounded degraded fallback, and provider prose is discarded.');
  requireText('src/features/product/runtime/repositoryActionFallback.ts', /classifyOfflineCapabilityIntent[\s\S]*case 'direct_patch'[\s\S]*default:[\s\S]*return null/, 'play-chat:degraded-fallback-runtime-boundary', 'The degraded repository fallback is a bounded runtime decision and fails closed for non-mutation intents.');
