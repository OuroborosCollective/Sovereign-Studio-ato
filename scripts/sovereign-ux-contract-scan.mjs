...
function run() {
  ...
  requireText(adapter, /\/api\/user\/agent\/single\/runs\/\$\{encodeURIComponent\(requested\)\}/, 'adapter:generic-run-readback', 'The generic adapter still supports persisted Swarm-run readback for non-repository product paths.');
