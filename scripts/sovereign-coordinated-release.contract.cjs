...
test('manual VPS bootstrap consumes one exact coordinated-release manifest instead of rebuilding the same revision', () => {
  assert.match(mcpWorkflow, /publish-mcp-image:[\s\S]*github\.event_name == 'push'[\s\S]*inputs\.publish_immutable_mcp == true/);
  assert.match(mcpWorkflow, /verify-published-mcp-image:[\s\S]*github\.event_name == 'push'[\s\S]*inputs\.publish_immutable_mcp == true/);
