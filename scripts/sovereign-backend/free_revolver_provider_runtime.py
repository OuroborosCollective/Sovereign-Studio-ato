...
def _current_certified_model_sources(
    query: Callable[..., Any],
    *,
    source_id: str,
) -> dict[str, str]:
    """Return current v3-certified ready models without exposing receipt bodies."""
    ...
    if not (
        ...
        return {}
    rows = query(
        """SELECT model.upstream_model_id, model.eligibility_source
    ...
    return {
        str(row.get("upstream_model_id") or ""): str(
            row.get("eligibility_source")
            or "managed-freellm-chat-canary-verified"
        )
        for row in rows
    ...
    *,
    ...
    def admin_discover_free_revolver_provider(source_id: str):
        ...
        try:
            ...
            certified_model_sources = _current_certified_model_sources(
                query,
                source_id=source_id,
            )
            preserved_model_ids = {
                str(model["modelId"])
                for model in models
                if str(model["modelId"]) in certified_model_sources
                and bool(model.get("generalChatCanaryRequired"))
                and not bool(model.get("freeEligible"))
            }
            eligible_models = [
                model
                for model in models
                if model["freeEligible"]
                or str(model["modelId"]) in preserved_model_ids
            ]
            activation_models = [
                model
                for model in models
                ...
                and str(model["modelId"]) not in preserved_model_ids
            ]
            connection = get_connection()
            try:
                with connection.cursor() as cursor:
                    for model in models:
                        ...
                        if str(model["modelId"]) in preserved_model_ids:
                            cursor.execute(
                                """UPDATE llm_revolver_provider_models
                                   SET free_eligible=true, status='ready', enabled=true,
                                       eligibility_source=%s,
                                       last_error_code=NULL, updated_at=NOW()
                                   WHERE source_id=%s::uuid
                                     AND upstream_model_id=%s""",
                                (
                                    certified_model_sources[str(model["modelId"])],
                                    source_id,
                                    model["modelId"],
                                ),
                            )
    ...
    def internal_discover_managed_freellm_provider(source_id: str):
        ...
        try:
            ...
            eligible_models = [
                model
                for model in models
                if bool(model.get("freeEligible"))
                or str(model["modelId"]) in preserved_model_ids
            ]
            activation_models = [
                model
