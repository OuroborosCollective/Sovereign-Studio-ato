...
The protected Agent Zero token is read only at request time. Agent Zero's A2A proxy
requires it in the tokenized A2A path; it is never placed in the JSON body, persisted
in a Sovereign receipt or included in raised error text.
"""
...
_A2A_TOKEN_RE: Final[re.Pattern[str]] = re.compile(r"^[A-Za-z0-9._~-]{16,512}$")
...
class AgentZeroA2AClient:
    ...
    def _transport(self) -> tuple[str, dict[str, str]]:
        try:
            token = self.config.read_protected_key()
        except AgentZeroRuntimeError as exc:
            raise AgentZeroA2AError(exc.family, exc.next_action, http_status=exc.http_status) from exc
        if not _A2A_TOKEN_RE.fullmatch(token):
            raise AgentZeroA2AError(
                "AGENT_ZERO_A2A_TOKEN_INVALID",
                "ROTATE_AGENT_ZERO_TOKEN_TO_URL_SAFE_VALUE",
            )
        return (
            f"{self.config.base_url.rstrip('/')}/a2a/t-{token}",
            {
    ...
    def _post_rpc(self, payload: dict[str, Any], *, submit: bool) -> Mapping[str, Any]:
        endpoint, headers = self._transport()
        try:
