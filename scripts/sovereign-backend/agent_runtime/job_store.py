...
def append_agent_projection(conn: Any, *, job_id: str, projection: Mapping[str, Any]) -> None:
    ...
    if payload.get("schemaVersion") != "sovereign.live-workspace-projection.v1":
        raise ValueError("unsupported live workspace projection schema")
    ...
    for row in rows:
        ...
        if isinstance(raw, str):
            try:
                ...
                continue
        if isinstance(raw, Mapping) and raw.get("schemaVersion") == "sovereign.live-workspace-projection.v1":
