...
def admin_llm_gateway_providers():
    providers = ["openai", "anthropic", "mistral", "cohere", "google", "azure"]
    results = []
    for provider in providers:
        has_key, error, models = test_provider_key(provider)
        results.append({
            "provider": provider,
            "configured": has_key,
            "error": error if not has_key else None,
            "modelCount": len(models) if has_key else 0,
            "models": [m.get("id", "") for m in models[:5]] if has_key else [],
        })
...
def admin_llm_gateway_sync():
    try:
        ...
        for provider in providers:
            has_key, error, models = test_provider_key(provider)
            if not has_key:
                results["providers"][provider] = {"configured": False, "error": error, "created": 0, "updated": 0}
                continue
            created = []
            updated = []
            for model in models:
                ...
                priority = 50
                if any(x in model_id.lower() for x in ["mini", "lite", "haiku"]):
                    priority = 10
                elif any(x in model_id.lower() for x in ["large", "ultra", "opus"]):
                    priority = 100
                try:
                    if existing:
                        query("UPDATE llm_routes SET model_name = %s, base_url = %s, priority = %s, disabled = false, updated_at = NOW() WHERE model_id = %s",
                              (model_name, AI_GATEWAY_BASE, priority, full_model_id), write=True)
                        updated.append(full_model_id)
                    else:
                        new_id = query("INSERT INTO llm_routes (id, model_id, model_name, provider, base_url, credits_per_unit, priority, disabled) VALUES (gen_random_uuid(), %s, %s, %s, %s, 0.01, %s, false) RETURNING id::text",
                                       (full_model_id, model_name, provider, AI_GATEWAY_BASE, priority), write=True, one=True)
                        created.append({"id": new_id["id"], "model": full_model_id})
                except Exception as e:
                    results["errors"].append(f"{full_model_id}: {str(e)}")
            results["providers"][provider] = {"configured": True, "modelCount": len(models), "created": len(created), "updated": len(updated)}
        
        try:
            resp, err = fetch_worker_ai("v1/models")
            if not err and resp and resp.ok:
                worker_models = resp.json().get("data", [])
                w_created, w_updated = 0, 0
                for wm in worker_models:
                    model_id = wm.get("id", "")
                    if not model_id:
                        continue
                    existing = query("SELECT id::text FROM llm_routes WHERE model_id = %s", (model_id,), one=True)
                    priority = 50
                    if "70b" in model_id or "32b" in model_id:
                        priority = 100
                    elif "7b" in model_id:
                        priority = 25
                    try:
                        if existing:
                            query("UPDATE llm_routes SET model_name = %s, base_url = %s, priority = %s, disabled = false, updated_at = NOW() WHERE model_id = %s",
                                  (model_id, WORKER_AI_BASE, priority, model_id), write=True)
                            w_updated += 1
                        else:
                            query("INSERT INTO llm_routes (id, model_id, model_name, provider, base_url, credits_per_unit, priority, disabled) VALUES (gen_random_uuid(), %s, %s, 'cloudflare', %s, 0.001, %s, false)",
                                  (model_id, model_id, WORKER_AI_BASE, priority), write=True)
                            w_created += 1
                    except:
                        pass
                results["workerAiSync"] = {"ok": True, "totalModels": len(worker_models), "created": w_created, "updated": w_updated}
        except Exception as e:
...
def public_llm_auto_route():
    try:
        ...
        if not routes:
            return jsonify({"error": "Keine LLM Routes verfuegbar", "suggestion": "Fuehren Sie /api/admin/llm/gateway/sync aus"}), 503
        selected = routes[0]
        ...
        has_key, error, _ = test_provider_key(provider)
        if not has_key:
            worker_routes = [r for r in routes if r.get("provider") == "cloudflare"]
            if worker_routes:
                return jsonify({"ok": True, "selected": {"modelId": worker_routes[0]["model_id"], "modelName": worker_routes[0]["model_name"], "provider": "cloudflare", "baseUrl": worker_routes[0]["base_url"], "creditsPerUnit": float(worker_routes[0]["credits_per_unit"]), "routeType": "worker-ai-fallback"}, "fallback": [], "message": f"Fallback to Worker AI (Provider {provider} key fehlt)"})
            return jsonify({"error": f"Provider {provider} key nicht konfiguriert"}), 503
...
def public_llm_chat():
    try:
        ...
        if model.startswith("@cf/"):
            ...
            return jsonify({"error": f"Unbekanntes Model Format: {model}"}), 400
    except Exception as e:
        return jsonify({"error": str(e)}), 500
