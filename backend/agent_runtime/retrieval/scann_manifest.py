...
class EmbeddingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.model_id) > _MAX_MODEL_ID_LEN:
            raise ManifestContractError(f"model_id exceeds {_MAX_MODEL_ID_LEN} chars")
...
class SourceRecordRef:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.source_file.startswith("/"):
            raise ManifestContractError(f"source_file must be absolute path")
...
class ScaNNBuildConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.pq_codebooks < 0:
            raise ManifestContractError("pq_codebooks must be >= 0")
        if self.reorder_window < 0:
            raise ManifestContractError("reorder_window must be >= 0")
...
@dataclass(frozen=True, slots=True)
class ChunkManifest:
    """Manifest for chunked index storage."""
    chunk_index: int
...
class VectorSnapshotManifest:
    ...
    def __post_init__(self) -> None:
        ...
        # Verify chunk manifest
        for chunk in self.chunk_manifest:
            if chunk.record_start >= chunk.record_end:
                raise ManifestContractError(
                    f"Chunk {chunk.chunk_index}: record_start >= record_end"
                )
    ...
    def total_hash(self) -> str:
        ...
        return _canonical_sha256({
            ...
            "record_hashes": [r.content_hash for r in self.source_records],
...
def check_embedding_drift(
    ...
    """
    return (
        manifest_config.provider == current_config.provider
        and manifest_config.model_id == current_config.model_id
        and manifest_config.dimension == current_config.dimension
        and manifest_config.normalization == current_config.normalization
        and manifest_config.distance_metric == current_config.distance_metric
    )
