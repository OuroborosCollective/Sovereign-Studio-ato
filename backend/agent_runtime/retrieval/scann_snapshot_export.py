...
def extract_bug_evidence_for_export(
    ...
    # Build normalized content from signature
    ...
    record_id = f"be:{case.get('evidence_case_id', '')}"
    source_file = f"/bug_evidence/{case.get('evidence_case_id', '')}"
...
def export_snapshot(
    ...
    # Sort records by record_id for deterministic ordering
    ...
    content_concat = "".join(r.normalized_content for r in records)
    content_hash = compute_content_hash(content_concat)
