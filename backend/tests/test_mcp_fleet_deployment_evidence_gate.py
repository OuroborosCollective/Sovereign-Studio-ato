...
class TestEvaluateBlocked:
    ...
    def test_empty_bound_post_restart_rollback_readback_blocked(self) -> None:
        ...
        obs = [
            ...
            for o in _full_observations("vps_deployment_restart_rollback")
        ]
        result = evaluate_mcp_fleet_evidence(env, obs)
        assert result.verdict == VERDICT_BLOCKED
    ...
    def test_post_restart_rollback_readback_bound_only_by_digest_satisfies(self) -> None:
        # post_restart_rollback_readback bound by digest only → no mismatch, satisfies.
        env = _envelope(family="vps_deployment_restart_rollback")
        obs = [
            o if o.requirement_id != "post_restart_rollback_readback"
            else McpFleetObservation(
