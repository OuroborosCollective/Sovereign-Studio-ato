...
class TestEvaluateBlocked:
    ...
    def test_post_restart_rollback_readback_empty_binding_blocked(self) -> None:
        # post_restart_rollback_readback present but bound_revision="" AND bound_digest=""
        # → no real prior/current image referenced → requirement remains unsatisfied.
        # Mirrors the empty-binding guard for pre_rollback_digest (Issue #1101).
        ...
        assert "post_restart_rollback_readback_lacks_revision_or_digest_binding" in result.finding_codes

    def test_post_restart_rollback_readback_bound_only_by_revision_satisfies(self) -> None:
        # post_restart_rollback_readback bound to base_revision only (no digest) → satisfies.
        # The guard must not reject a one-sided binding; it only rejects an empty one.
        env = _envelope(family="host_patch_sovereign_runtime")
        obs = [
            o if o.requirement_id != "post_restart_rollback_readback"
            else McpFleetObservation(
        ...
        assert "post_restart_rollback_readback_lacks_revision_or_digest_binding" not in result.finding_codes

    def test_post_restart_rollback_readback_bound_only_by_digest_satisfies(self) -> None:
        # post_restart_rollback_readback bound by digest only (no revision) → satisfies.
        # The guard must not reject a one-sided binding; it only rejects an empty one.
        env = _envelope(family="host_patch_sovereign_runtime")
        obs = [
            o if o.requirement_id != "post_restart_rollback_readback"
