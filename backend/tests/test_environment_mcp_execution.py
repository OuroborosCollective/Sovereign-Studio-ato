...
class TestEgressPolicyEngine:
    ...
    def test_subdomain_of_allowlisted_host_allowed_with_dns_evidence(self):
    ...
    def test_allowed_hostname_requires_dns_evidence(self):
    ...
    def test_egress_receipt_is_deterministic(self):
        manifest = _make_manifest(allowed_egress_hosts=("api.openai.com",))
        first = EgressPolicyEngine.decide(
            environment_manifest=manifest,
            target_host="api.openai.com",
            protocol="https",
            resolved_ip="104.18.33.45",
        )
        second = EgressPolicyEngine.decide(
            environment_manifest=manifest,
            target_host="api.openai.com",
            protocol="https",
            resolved_ip="104.18.33.45",
...
class TestMcpInstallationBinder:
    ...
    def test_different_installations_have_different_hashes(self):
        b1 = McpInstallationBinder.bind(
            tool_id="tool", server_id="server", installation_id="install-A", registry_revision=_SHA
        )
        b2 = McpInstallationBinder.bind(
            tool_id="tool", server_id="server", installation_id="install-B", registry_revision=_SHA
        )
...
class TestExecutionIdentityReceiptBuilder:
    ...
    def test_tampered_principal_receipt_is_rejected(self):
        import dataclasses

        manifest, principal, credential, egress, binding = self._setup()
        tampered = dataclasses.replace(principal, principal_id="attacker")
        with pytest.raises(EnvironmentContractError, match="hash verification"):
            ExecutionIdentityReceiptBuilder.build(
                run_id="run-001",
                environment_manifest=manifest,
                principal_receipt=tampered,
                credential_receipt=credential,
                egress_receipt=egress,
                installation_binding=binding,
            )

    def test_mismatched_credential_owner_is_rejected(self):
        manifest, principal, _, egress, binding = self._setup()
        credential = CredentialResolver.resolve(
            environment_id=manifest.environment_id,
            credential_id="key-v2",
    ...
    def test_mismatched_run_is_rejected(self):
        manifest, _, credential, egress, binding = self._setup()
    ...
    def test_installation_revision_drift_is_rejected(self):
        manifest, principal, credential, egress, _ = self._setup()
        binding = McpInstallationBinder.bind(
            tool_id="worker-tool",
            server_id="mcp-server",
            installation_id="install-001",
            registry_revision=_SHA,
            verified_at_revision=_SHA2,
        )
        with pytest.raises(EnvironmentContractError, match="Installation binding revision"):
            ExecutionIdentityReceiptBuilder.build(
                run_id="run-001",
                environment_manifest=manifest,
                principal_receipt=principal,
    ...
    def test_execution_receipt_is_deterministic(self):
        manifest, principal, credential, egress, binding = self._setup()
...
class TestEnvironmentMcpMigrationContract:
    @staticmethod
    def _migration_text() -> str:
        return (ROOT / "backend/migrations/049_environment_mcp_execution.sql").read_text()

    def test_deployment_migration_is_byte_identical(self):
        canonical = ROOT / "backend/migrations/049_environment_mcp_execution.sql"
        mirror = ROOT / "scripts/sovereign-backend/migrations/049_environment_mcp_execution.sql"
        assert canonical.read_bytes() == mirror.read_bytes()

    def test_receipt_identity_has_no_serial_clock_defaults(self):
        sql = self._migration_text()
        assert "BIGSERIAL" not in sql
        assert "DEFAULT NOW()" not in sql
        assert "DEFAULT FALSE" not in sql

    def test_allow_receipt_requires_dns_ip_evidence(self):
        sql = self._migration_text()
        assert "resolved_ip         INET" in sql
        assert "decision = 'allow' AND block_reason IS NULL AND resolved_ip IS NOT NULL" in sql
        assert "EXECUTION_EGRESS_NOT_VERIFIED" in sql

    def test_execution_identity_cross_links_are_enforced(self):
        sql = self._migration_text()
        for marker in (
            "EXECUTION_ENVIRONMENT_ID_MISMATCH",
            "EXECUTION_RUN_ID_MISMATCH",
            "EXECUTION_PRINCIPAL_REVISION_MISMATCH",
            "EXECUTION_CREDENTIAL_IDENTITY_MISMATCH",
            "EXECUTION_ENVIRONMENT_KIND_MISMATCH",
            "EXECUTION_TOOL_BINDING_MISMATCH",
            "EXECUTION_INSTALLATION_REVISION_MISMATCH",
        ):
            assert marker in sql

    def test_all_truth_tables_are_immutable(self):
        sql = self._migration_text()
        assert sql.count("BEFORE UPDATE OR DELETE ON") == 6
        assert sql.count("REVOKE UPDATE, DELETE ON") == 6
        assert "IMMUTABLE_EXECUTION_RECEIPT_STORE" in sql
