...
def test_submit_sets_both_auth_headers_keeps_secret_out_of_body_and_is_nonblocking(monkeypatch, tmp_path: Path):
    ...
    assert calls[0]["url"] == f"https://agent-zero.example.invalid/a2a/t-{token}"
...
def test_non_url_safe_a2a_token_is_rejected_before_transport(monkeypatch, tmp_path: Path):
    config = _config(monkeypatch, tmp_path, "agent-zero/a2a-invalid-token-0123456789")
    called = False
    ...
    assert error.value.family == "AGENT_ZERO_A2A_TOKEN_INVALID"
    assert "agent-zero/a2a-invalid-token" not in str(error.value)
