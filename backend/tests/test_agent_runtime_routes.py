...
class FakeCursor:
    ...
    def execute(self, sql, params=None):
        ...
        if normalized.startswith("INSERT INTO SOVEREIGN_AGENT_JOBS"):
            self.conn.jobs[params[1]] = {
                "user_id": params[0], "job_id": params[1], "executor": params[2],
                "repo_url": params[3], "branch": params[4], "mission": params[5],
                "status": params[6], "workspace_id": params[7], "allowed_paths": params[8],
                "forbidden_paths": params[9], "memory_hints": params[10], "external_ref": None,
                "draft_pr_url": None, "changed_files": [], "diff_summary": None,
                "test_summary": None, "events": params[11], "blocker": params[12],
            }
            ...
            if params[1]: self.conn.jobs[job_id]["workspace_id"] = params[1]
            if params[2]: self.conn.jobs[job_id]["external_ref"] = params[2]
            if params[3]: self.conn.jobs[job_id]["changed_files"] = params[3]
            if params[4]: self.conn.jobs[job_id]["diff_summary"] = params[4]
            if params[5]: self.conn.jobs[job_id]["test_summary"] = params[5]
            if params[6]: self.conn.jobs[job_id]["draft_pr_url"] = params[6]
            if params[7]: self.conn.jobs[job_id]["blocker"] = params[7]
    ...
    def fetchone(self): return self.last_result if isinstance(self.last_result, dict) else None
    def fetchall(self): return self.last_result if isinstance(self.last_result, list) else []
...
def seed_job(conn: FakeConnection, user_id: str, job_id: str, status: str = "queued"):
    create_agent_job_record(
        conn, user_id=user_id, job_id=job_id, request=valid_request(), status=status,
...
def test_routes_require_session():
    conn = FakeConnection(); app = create_test_app(conn)
...
def test_list_jobs_is_user_scoped():
    conn = FakeConnection(); seed_job(conn, "user-1", "agent-1"); seed_job(conn, "user-2", "agent-2")
    response = create_test_app(conn).test_client().get("/api/user/agent/jobs", headers={"X-Test-User": "user-1"})
...
def test_create_job_runs_lifecycle_and_returns_runtime_state(tmp_path, monkeypatch):
    conn = FakeConnection(); monkeypatch.setenv("SOVEREIGN_AGENT_WORKSPACE_ROOT", str(tmp_path)); app = create_test_app(conn)
    response = app.test_client().post(
        "/api/user/agent/jobs", headers={"X-Test-User": "user-1"},
        json={"repoUrl": "https://github.com/OuroborosCollective/Sovereign-Studio-ato", "branch": "main", "mission": "Update README and prepare a Draft PR.", "provisionWorkspace": True, "cloneRepo": False},
    )
    payload = response.get_json()
    assert response.status_code == 201 and payload["ok"] is True
    assert payload["runtime"] == "sovereign-agent"
...
def test_create_invalid_job_returns_blocked_without_fake_success(tmp_path, monkeypatch):
    ...
    response = app.test_client().post(
        "/api/user/agent/jobs", headers={"X-Test-User": "user-1"},
        json={"repoUrl": "https://evil.example/repo", "mission": "Do unsafe work."},
    )
    payload = response.get_json()
...
def test_get_job_is_user_scoped():
    conn = FakeConnection(); seed_job(conn, "user-1", "agent-1"); app = create_test_app(conn)
    ...
    assert owned.status_code == 200 and owned.get_json()["job"]["jobId"] == "agent-1"
    assert other.status_code == 404


def test_cancel_non_terminal_job_sets_blocked():
    conn = FakeConnection(); seed_job(conn, "user-1", "agent-1", status="running"); app = create_test_app(conn)
...
def test_cancel_terminal_job_is_blocked():
    conn = FakeConnection(); seed_job(conn, "user-1", "agent-1", status="blocked"); app = create_test_app(conn)
    response = app.test_client().post("/api/user/agent/jobs/agent-1/cancel", headers={"X-Test-User": "user-1"})
...
def test_cleanup_terminal_job_sets_cleaned(tmp_path, monkeypatch):
    conn = FakeConnection(); monkeypatch.setenv("SOVEREIGN_AGENT_WORKSPACE_ROOT", str(tmp_path))
    seed_job(conn, "user-1", "agent-1", status="blocked"); (tmp_path / "agent-1" / "repo").mkdir(parents=True)
    response = create_test_app(conn).test_client().post("/api/user/agent/jobs/agent-1/cleanup", headers={"X-Test-User": "user-1"})
    assert response.status_code == 200
...
def test_janitor_scan_is_user_scoped_read_only_and_keeps_job_running(tmp_path, monkeypatch):
    ...
    repo = tmp_path / "agent-janitor" / "repo"; repo.mkdir(parents=True)
    source = "import subprocess\nsubprocess.run('echo unsafe', shell=True)\n"
    target = repo / "worker.py"; target.write_text(source, encoding="utf-8")
    app = create_test_app(conn)
    response = app.test_client().post(
        "/api/user/agent/jobs/agent-janitor/tools/janitor",
        headers={"X-Test-User": "user-1"}, json={"mode": "scan", "maxFindings": 10},
    )
    payload = response.get_json()
    assert response.status_code == 200 and payload["ok"] is True
    ...
    other_user = app.test_client().post(
        "/api/user/agent/jobs/agent-janitor/tools/janitor",
        headers={"X-Test-User": "user-2"}, json={"mode": "scan"},
    )
