...
def auth_github():
    ...
    try:
        ...
        if not GITHUB_CLIENT_ID or not GITHUB_CLIENT_SECRET:
            return jsonify({"error": "GitHub OAuth nicht konfiguriert"}), 503
        ...
        if not token_resp.ok:
            return jsonify({"error": "GitHub Token exchange fehlgeschlagen"}), 502
        ...
        # Wenn keine public email, versuche emails endpoint
        if not email:
        ...
        # Token verschlüsselt speichern (NIEMALS im Klartext!)
...
# Token-Verschlüsselung für GitHub Access Tokens
_GITHUB_TOKEN_KEY = os.getenv("GITHUB_TOKEN_ENCRYPTION_KEY", JWT_SECRET)
# Generiere einen stabilen Key aus dem Secret
_fernet_key = base64.urlsafe_b64encode(
    hashlib.sha256(_GITHUB_TOKEN_KEY.encode()).digest()
)
